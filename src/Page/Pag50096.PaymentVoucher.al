...
page 50096 "Payment Voucher"
{
    ...
    layout
    {
        area(content)
        {
...

            {
                ...
                field("Payment Requisition Form"; Rec."Payment Requisition Form")
                {
                    Caption = 'Payment Request No.';
                    trigger OnValidate()
                    var
                        Purchase: Record "Payments Header";
                        PaymentLine: Record "Payment Line";
                        PaymentLine2: Record "Payment Line";
                        PaymentLine3: Record "Payment Line";
                    begin
                        Purchase.Reset();
                        Purchase.SetRange("No.", Rec."Payment Requisition Form");
                        if Purchase.findset then begin

                            PaymentLine.Reset();
                            PaymentLine.SetRange(PaymentLine.No, Purchase."No.");
                            if PaymentLine.find('-') then begin
                                repeat

                                    PaymentLine2.Init();
                                    PaymentLine2.SetRange(No, PaymentLine2.No);
                                    if PaymentLine2.FindLast() then
                                        PaymentLine2."Line No." := PaymentLine2."Line No." + 1
                                    else
                                        PaymentLine2."Line No." := 0;
                                    PaymentLine2.No := Rec."No.";
                                    PaymentLine2.Type := PaymentLine.Type;
                                    PaymentLine2."Account Type" := PaymentLine."Account Type";
                                    PaymentLine2."Account No." := PaymentLine."Account No.";
                                    PaymentLine2."Account Name" := PaymentLine."Account Name";
                                    PaymentLine2."Global Dimension 1 Code" := PaymentLine."Global Dimension 1 Code";
                                    PaymentLine2."Shortcut Dimension 2 Code" := PaymentLine."Shortcut Dimension 2 Code";
                                    PaymentLine2.Amount := PaymentLine.Amount;
                                    PaymentLine2.Remarks := PaymentLine.Remarks;
                                    PaymentLine2."Net Amount" := PaymentLine."Net Amount";
                                    PaymentLine2.insert(true);
                                // Message('%1', PaymentLine2.No);
                ...
                field("Charge mpesa"; Rec."Charge mpesa")
                {
                    Caption = 'Charge Mpesa Fee';
                }
                ...
                field("Cheque No."; Rec."Cheque No.")
                {
                    ...
                    begin
                        ...
                        GLEntry.SetRange("Bank Account No.",Rec."Paying Bank Account");
            ...
            group("&Functions")
            {
                ...
                action(Approval)
                {
                    ...
                    begin
                        ...
                        ApprovalEntries.SetRecordFilters(Table_id, DocumentType, rec."No.");
                        ApprovalEntries.RUN;
                    end;
                }
                action("Re-open for Re-use")
                {
                    Promoted = true;
                    PromotedCategory = Category4;
                    PromotedOnly = true;
                    ...
                    begin
                        if Confirm('Are you sure you want to reopen the document?') = false then exit;
                        if UserId <> Rec.Cashier then Error('you can only open your documents alone');
                ...
                action(Print)
                {
                    ...
                    begin
                        ...
                        IF PHeader2.FINDFIRST THEN
                            Report.run(50082, TRUE, TRUE, PHeader2);
                    end;
                }
    ...
    begin
        ...
        Rec."Charge mpesa" := Rec."Charge mpesa";
        rec.Date := TODAY;
    end;
    ...
    var
        MobilePosting: Codeunit "Mobile Banking Code";
    begin
        ...
        IF Payments.FIND('-') THEN BEGIN
            ...
            IF Payments.FIND('-') THEN BEGIN
                ...
                IF PayLine.FIND('-') THEN BEGIN
                    ...
                    //  MobilePosting.ProcesstoMPESA(PayLine.No, PayLine.Remarks, PayLine."Mobile Phone No", PayLine."Net Amount", PayLine.Payee);
    ...
    begin
        ...
        IF Payments."Pay Mode" = Payments."Pay Mode"::Mpesa THEN BEGIN
            IF Payments."Cheque No." = '' THEN BEGIN
                Message('You May Proceed Without the Mpesa Ref. number.');
            END;
        END;
        ...
        if ((Payments."Pay Mode" = Payments."Pay Mode"::Mpesa) and (Payments."Charge mpesa" = True)) then begin
            GenJnlLine.Amount := (Payments."Total Net Amount" + Payments."Total VAT Amount" - GeneralSet."M-Pesa Charges PV") * -1;

        end else
            if ((Payments."Pay Mode" = Payments."Pay Mode"::EFT)) then begin
                GenJnlLine.Amount := (Payments."Total Net Amount" + Payments."Total VAT Amount" - GeneralSet." EFT Charges PV") * -1
            end else
                if ((Payments."Pay Mode" = Payments."Pay Mode"::RTGS)) then begin
                    GenJnlLine.Amount := (Payments."Total Net Amount" + Payments."Total VAT Amount" - GeneralSet."RTGS Charges PV") * -1;
                end else begin
        ...
        GenJnlLine.Description := COPYSTR('Pay To:' + Payments.Payee, 1, 150);
        ...
        IF ((Payments."Pay Mode" = Payments."Pay Mode"::Mpesa) and (Payments."Charge mpesa" = True)) THEN BEGIN
            ...
            ELSE
                GenJnlLine."Document Type" := GenJnlLine."Document Type"::" ";
            GenJnlLine."Document No." := Payments."No.";
            GenJnlLine."External Document No." := Payments."No.";
            GenJnlLine."Account Type" := GenJnlLine."Account Type"::"G/L Account";
            GenJnlLine."Account No." := GeneralSet."Mpesa A/c";
            GenJnlLine.VALIDATE(GenJnlLine."Account No.");
            GenJnlLine."Currency Code" := Payments."Currency Code";
            GenJnlLine.VALIDATE(GenJnlLine."Currency Code");
            //CurrFactor
            GenJnlLine."Currency Factor" := Payments."Currency Factor";
            GenJnlLine.VALIDATE("Currency Factor");
            GenJnlLine.Amount := (GeneralSet."M-Pesa Charges PV") * -1;
            GenJnlLine.VALIDATE(GenJnlLine.Amount);
            GenJnlLine."Shortcut Dimension 1 Code" := PayLine."Global Dimension 1 Code";
            GenJnlLine.VALIDATE(GenJnlLine."Shortcut Dimension 1 Code");
            GenJnlLine."Shortcut Dimension 2 Code" := PayLine."Shortcut Dimension 2 Code";
            GenJnlLine.VALIDATE(GenJnlLine."Shortcut Dimension 2 Code");
            GenJnlLine.Description := COPYSTR('mpesa charges:' + Payments.Payee, 1, 150);
            GenJnlLine.VALIDATE(GenJnlLine.Description);
        ...
        IF (Payments."Pay Mode" = Payments."Pay Mode"::EFT) THEN BEGIN
            ...
            GenJnlLine."Currency Factor" := Payments."Currency Factor";
            GenJnlLine.VALIDATE("Currency Factor");
            GenJnlLine.Amount := (GeneralSet." EFT Charges PV") * -1;
            GenJnlLine.VALIDATE(GenJnlLine.Amount);
            GenJnlLine."Shortcut Dimension 1 Code" := PayLine."Global Dimension 1 Code";
            GenJnlLine.VALIDATE(GenJnlLine."Shortcut Dimension 1 Code");
            GenJnlLine."Shortcut Dimension 2 Code" := PayLine."Shortcut Dimension 2 Code";
            GenJnlLine.VALIDATE(GenJnlLine."Shortcut Dimension 2 Code");
            GenJnlLine.Description := COPYSTR('Eft charges:' + Payments.Payee, 1, 150);
            GenJnlLine.VALIDATE(GenJnlLine.Description);
        ...
        IF (Payments."Pay Mode" = Payments."Pay Mode"::RTGS) THEN BEGIN
            ...
            GenJnlLine."External Document No." := Payments."No.";
            GenJnlLine."Account Type" := GenJnlLine."Account Type"::"G/L Account";
            GenJnlLine."Account No." := GeneralSet."Loan Trasfer Fee A/C-RTGS";
            GenJnlLine.VALIDATE(GenJnlLine."Account No.");
            GenJnlLine."Currency Code" := Payments."Currency Code";
            GenJnlLine.VALIDATE(GenJnlLine."Currency Code");
            //CurrFactor
            GenJnlLine."Currency Factor" := Payments."Currency Factor";
            GenJnlLine.VALIDATE("Currency Factor");
            GenJnlLine.Amount := (GeneralSet."RTGS Charges PV") * -1;
            GenJnlLine.VALIDATE(GenJnlLine.Amount);
            GenJnlLine."Shortcut Dimension 1 Code" := PayLine."Global Dimension 1 Code";
            GenJnlLine.VALIDATE(GenJnlLine."Shortcut Dimension 1 Code");
            GenJnlLine."Shortcut Dimension 2 Code" := PayLine."Shortcut Dimension 2 Code";
            GenJnlLine.VALIDATE(GenJnlLine."Shortcut Dimension 2 Code");
            GenJnlLine.Description := COPYSTR('RTGS charges:' + Payments.Payee, 1, 150);
            GenJnlLine.VALIDATE(GenJnlLine.Description);
