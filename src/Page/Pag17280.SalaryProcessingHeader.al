...
Page 17280 "Salary Processing Header"
{
    PageType = Card;
                    ...
                    begin
                        ...
                        if salarybuffer.Find('-') then begin
                            ...
                            repeat
                                ...
                                if Rec."Transaction Type" = Rec."transaction type"::Salary then
                        ...
                        if ObjLoanProducts."Recovery Method" = ObjLoanProducts."recovery method"::"Salary " then begin
                            ...
                            if (SFactory.FnGetOutstandingInterest(LoanApp."Loan  No.")) > 0 then begin
                                if RunningBalance > 0 then begin
                                    ...
                                    LineNo := LineNo + 10000;
                                    SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::"Interest Paid",
                                    GenJournalLine."account type"::None, ObjRcptBuffer."Account No.", Rec."Posting date", AmountToDeduct * -1, Format(LoanApp.Source), EXTERNAL_DOC_NO,
                                    Format(GenJournalLine."transaction type"::"Interest Paid"), LoanApp."Loan  No.", GenJournalLine."application source"::" ");
                            ...
                            if RunningBalance > 0 then begin
                                ...
                                if LoanApp."Outstanding Balance" > 0 then begin
                                    ...
                                    if varLRepayment > 0 then begin
                                        ...
                                        LineNo := LineNo + 10000;
                                        SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."Transaction Type"::Repayment,
                                        GenJournalLine."account type"::None, ObjRcptBuffer."Member No", Rec."Posting date", AmountToDeduct * -1, Format(LoanApp.Source), EXTERNAL_DOC_NO,
                                        Format(GenJournalLine."Transaction Type"::Repayment), LoanApp."Loan  No.", GenJournalLine."application source"::" ");
                                        //-------------RECOVER---------------
                                        LineNo := LineNo + 10000;
                                        SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
    ...
    begin
        ...
        BATCH_NAME := 'SALARIES';
        GetBatch.Reset();
        GetBatch.SetRange("Journal Template Name", BATCH_TEMPLATE);
        GetBatch.SetRange(Name, BATCH_NAME);
        if GetBatch.FindFirst() = false then begin
            GetBatch.Name := BATCH_NAME;
            GetBatch."Journal Template Name" := BATCH_TEMPLATE;
            GetBatch.insert();
        end;
        DOCUMENT_NO := Rec."Document No";
        EXTERNAL_DOC_NO := "Cheque No.";
        ...
        Rec.Modify;
        // if Rec."Transaction Type" = Rec."transaction type"::Salary then begin
    ...
    begin
        ...
        if ObjSalaryProcessingLines.FindSet then begin
            repeat
                ...
                Cust.Reset();
                Cust.SetRange("FOSA Account No.", ObjSalaryProcessingLines."Account No.");
                ...
                Emailmessage.create(Email, 'Salary Processed', FormattedBody, true);
