...
Page 50012 "Member Account Card"
{
    Caption = 'Member Card';
    DeleteAllowed = false;
    InsertAllowed = false;
        ...
        area(content)
        {
            group("General Info")
            {
                ...
                field("Loan Status"; Rec."Loan Status")
                {
                    ...
                    // begin
                    //     if Rec."Block Mobile Loan" = false then begin
                    //         UserSetup.Reset;
                ...
                field("Special Accounts"; Rec."Special Accounts")
                {
                    ApplicationArea = Basic;
                    Editable = true;
                ...
                field("Online Member"; Rec."Online Member")
                {
                    ApplicationArea = Basic;
                    Editable = false;
                    Visible = false;
                ...
                field("Block Normal Loan"; Rec."Block Normal Loan")
                {
                    ApplicationArea = Basic;
                    ...
                    var
                        UserSetup: Record "User Setup";
                    begin
                        if Rec."Block Normal Loan" = false then begin
                            UserSetup.Reset;
                            UserSetup.SetRange(UserSetup."User ID", UserId);
                            UserSetup.SetRange(UserSetup."Unblock Loan Application", false);
                            if UserSetup.FindSet then
            ...
            // {
            //     field("Type of house"; Rec."Type of house")
            //     {
            //         ApplicationArea = basic;
            //     }
            //     field("HouseHold Size"; Rec."HouseHold Size") { ApplicationArea = all; }
            //     field("Access to Clean water"; Rec."Access to Clean water") { ApplicationArea = basic; }
            //     field("Monthly Income"; Rec."Monthly Income") { ApplicationArea = all; }
            // }
            group("Bank Details")
            {
        ...
        area(navigation)
        {
            group("&Member")
            {
                ...
                action("Exempt 3 Months Policy")
                {
                    ...
                    trigger OnAction()
                    begin
                        if Confirm('Are you sure you want to Exempt?') = false then exit;
                        begin
                            Rec."Exempt 3 Months Policy" := true;
                            Rec."Exempted By" := UserId;
                            Rec.Modify(true);
                        end;
                    end;
            ...
            group(ActionGroup1102755023)
            {
                ...
                action("Create Withdrawal Application")
                {
                    ...
                    begin
                        ...
                        if Confirm('Are you sure you want to create a Withdrawal Application for this Member', false) = true then begin
                            if (Rec.status = Rec.status::"Awaiting Exit") or (Rec.status = Rec.status::Deceased) or (Rec.status = Rec.status::Exited) then
                                Error('Members withdrawal process is only applicable to active or dormant members. member status %1', Rec.Status);
                            AUFactory.FnCreateMembershipWithdrawalApplication(Rec."No.", Rec."Withdrawal Application Date", Rec."Reason For Membership Withdraw", Rec."Withdrawal Date");
                        end;
                    end;
                }
                ...
                action("Send Member Statement")
                {
                    ...
                    var
                        ...
                        DetailedStatement: Report "Detailed Statement Via Email";
                    begin
                        Cust.Reset;
                        Cust.SetRange("No.", Rec."No.");
                        if Cust.Find('-') then begin
                            Report.Run(50071, true, false, Cust);
                            // DetailedStatement.SetCustomerNo(Cust."No.");
                            // DetailedStatement.SetDateFilter('01/01/24..11/03/25'); // Your test case
                            // DetailedStatement.SetEmailAddress(Cust."E-Mail");      // Optional, falls back to Customer email
                            // DetailedStatement.RunModal();                          // No request page
                        end else
                            Error('Customer %1 not found.', Rec."No.");
                    end;
                }
                ...
                action("Programmed  Slip")
                {
                    ApplicationArea = Basic;
                    Caption = 'Programmed Deposits Slip';
                    Image = "Report";
                    Promoted = true;
                    PromotedCategory = "Report";
                    PromotedIsBig = true;
                    ...
                    begin
                        ...
                        Cust.SetRange(Cust."No.", Rec."No.");
                        if Cust.Find('-') then
                            Report.Run(50028, true, false, Cust);
                    end;
                }
                ...
                action("Membership report form")
                {
                    ...
                    trigger OnAction()
                    var
                        Cust: Record "Membership Applications";
                    begin
                        Cust.Reset;
                        Cust.SetRange(Cust."No.", Rec."No.");
                        if Cust.Find('-') then
                            Report.Run(80052, true, false, Cust);
                    end;
                }
    ...
    begin
        ...
        if Rec."Account Category" <> Rec."account category"::Individual then begin
            ...
            Rec.Age := Rec.CalculateAge(WorkDate());
            Rec.Modify();
        end;
    end;
