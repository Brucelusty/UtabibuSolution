//************************************************************************
Codeunit 50071 "WorkflowIntegration"
{
    trigger OnRun()
    begin
    ...
    procedure CheckJobApprovalsWorkflowEnabled(var HRJobss: Record "HR Jobss"): Boolean
    begin
        if not IsJobApprovalsWorkflowEnabled(HRJobss) then
            Error(NoWorkflowEnabledErr);
        exit(true);
    end;


    procedure IsJobApprovalsWorkflowEnabled(var HRJobss: Record "HR Jobss"): Boolean
    begin
        exit(WorkflowManagement.CanExecuteWorkflow(HRJobss, SurestepWFEvents.RunWorkflowOnSendJobApplicationForApprovalCode()));
    end;

    [IntegrationEvent(false, false)]

    procedure OnSendJobApplicationForApproval(var HRJobss: Record "HR Jobss")
    begin
    end;

    [IntegrationEvent(false, false)]

    procedure OnCancelJobApplicationApprovalRequest(var HRJobss: Record "HR Jobss")
    begin
    end;
    //end
    ...
    procedure CheckHREmployeeRequisitionApprovalsWorkflowEnabled(var HREmployeeRequisitions: Record "HR Employee Requisitions"): Boolean
    begin
        if not IsHREmployeeRequisitionApprovalsWorkflowEnabled(HREmployeeRequisitions) then
            Error(NoWorkflowEnabledErr);
        exit(true);
    end;


    procedure IsHREmployeeRequisitionApprovalsWorkflowEnabled(var HREmployeeRequisitions: Record "HR Employee Requisitions"): Boolean
    begin
        exit(WorkflowManagement.CanExecuteWorkflow(HREmployeeRequisitions, SurestepWFEvents.RunWorkflowOnSendHREmployeeRequisitionForApprovalCode()));
    end;

    [IntegrationEvent(false, false)]

    procedure OnSendHREmployeeRequisitionApplicationForApproval(var HREmployeeRequisitions: Record "HR Employee Requisitions")
    begin
    end;

    [IntegrationEvent(false, false)]

    procedure OnCancelHREmployeeRequisitionApplicationApprovalRequest(var HREmployeeRequisitions: Record "HR Employee Requisitions")
    begin
    end;
    //end

    // Training Requisition
    procedure CheckHRTrainingApprovalsWorkflowEnabled(var HRTrainingApplications: Record "HR Training Applications"): Boolean
    begin
        if not IsHRTrainingApprovalsWorkflowEnabled(HRTrainingApplications) then
            Error(NoWorkflowEnabledErr);
        exit(true);
    end;


    procedure IsHRTrainingApprovalsWorkflowEnabled(var HRTrainingApplications: Record "HR Training Applications"): Boolean
    begin
        exit(WorkflowManagement.CanExecuteWorkflow(HRTrainingApplications, SurestepWFEvents.RunWorkflowOnSendHRTrainingForApprovalCode()));
    end;

    [IntegrationEvent(false, false)]

    procedure OnSendHRTrainingApplicationForApproval(var HRTrainingApplications: Record "HR Training Applications")
    begin
    end;

    [IntegrationEvent(false, false)]

    procedure OnCancelHRTrainingApplicationApprovalRequest(var HRTrainingApplications: Record "HR Training Applications")
    begin
    end;
    ...
    procedure CheckboardpaymentApprovalsWorkflowEnabled(var boardpayment: Record "Boad Payments Allowance"): Boolean
    begin
        if not IsboardpaymentApprovalsWorkflowEnabled(boardpayment) then
            Error(NoWorkflowEnabledErr);
        exit(true);
    end;

    procedure IsboardpaymentApprovalsWorkflowEnabled(var boardpayment: Record "Boad Payments Allowance"): Boolean
    begin
        exit(WorkflowManagement.CanExecuteWorkflow(boardpayment, SurestepWFEvents.RunWorkflowOnSendboardpaymentForApprovalCode()));
    end;

    [IntegrationEvent(false, false)]

    procedure OnSendboardpaymentForApproval(var boardpayment: Record "Boad Payments Allowance")
    begin
    end;

    [IntegrationEvent(false, false)]

    procedure OnCancelboardpaymentApprovalRequest(var boardpayment: Record  "Boad Payments Allowance")
    begin
    end;
    ...
    begin
        case RecRef.Number of
            ...
            //Emploee Requisition Applications
            Database::"HR Employee Requisitions":
                begin
                    RecRef.SetTable(HREmployeeRequisitions);
                    ApprovalEntryArgument."Document Type" := ApprovalEntryArgument."document type"::HREmployeeRequisition;
                    ApprovalEntryArgument."Document No." := HREmployeeRequisitions."Requisition No.";
            ...
            Database::"Boad Payments Allowance":
                begin
                    ...
                    ApprovalEntryArgument."Document Type" := ApprovalEntryArgument."document type"::boardpayments;
                    ApprovalEntryArgument."Document No." := boardpayment.No;
            ...
            Database::"EFT/RTGS Header":
                begin
                    ...
                    // := EFTRTGS."Transaction Description";
            ...
            Database::"HR Jobss":
                begin
                    ...
                    ApprovalAmount := 0;
                    ApprovalAmountLCY := 0;
                    ApprovalEntryArgument."Document Type" := ApprovalEntryArgument."document type"::HRJob;
                    ApprovalEntryArgument."Document No." := hrJobs."Job ID";

                    ApprovalEntryArgument.Amount := hrJobs."Vacant Positions";
            ...
            Database::"HR Training Applications":
                begin
                    ...
                    ApprovalEntryArgument."Document Type" := ApprovalEntryArgument."document type"::TrainingNeeds;
                    ApprovalEntryArgument."Document No." := HRTrainingApplications."Application No";
                    ApprovalEntryArgument.Amount := HRTrainingApplications."Approved Cost";
