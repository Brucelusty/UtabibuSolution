...
Codeunit 50020 "Au Factory"
{
    ...
    begin
        ...
        SMSMessage."Entered By" := UserID;
        SMSMessage."Sent To Server" := SMSMessage."sent to server"::No;
        SMSMessage."SMS Message" := SMSBody + '. Shofco Sacco';
        SMSMessage."Telephone No" := MobileNumber;
    ...
    begin
        ...
        SMSMessage."Entered By" := UserId;
        SMSMessage."Sent To Server" := SMSMessage."sent to server"::No;
        SMSMessage."SMS Message" := SMSBody + '. Shofco Sacco';// +' '+ObjGenSetUp."Customer Care No";
        SMSMessage."Telephone No" := MobileNumber;
    ...
    begin
        ...
        GenJournalLine.Validate(GenJournalLine."Source No.");
            ...
            if (LoansRec."Loan Product Type" = 'DEFAULTER') and (LoansRec."Issued Date" <> 0D) and (LoansRec."Repayment Start Date" <> 0D) then begin
                ...
                repeat
                    ...
                    if LoansRec."Repayment Method" = LoansRec."repayment method"::"Straight Line" then begin
                        ...
                        if LoansRec."Loan Product Type" = 'INSTANT' then begin

                            LInterest := (3.333 / 100) * LoanAmount;
                        end else begin

                            LInterest := Round((InterestRate / 1200) * LoanAmount / RepayPeriod, 1, '>');
                        end;
                    end;
    ...
    procedure FnGetPAYEBudCharge(ChargeCode: Code[10]) Amount: Decimal
    var
    ...
    var
        ...
        Taxrelief: Decimal;
        OTrelief: Decimal;
        PAYESetup: Record "PAYE Brackets Credit";
        TaxablePay2: Decimal;
        Count: integer;
        TempAmount: decimal;
    begin
        ...
        if TAXABLEPAY.Find('-') then begin
                    ...
                    case TAXABLEPAY."Tax Band" of
                        '01':
                            begin
                                ...
                                Message('HOME1 %1-%2 -%3-%4', Chargeable, BAND1, TAXABLEPAY."Taxable Amount", TAXABLEPAY."Upper Limit");
            ...
            PAYE := PAYE + BAND1 + BAND2 + BAND3 + BAND4 + BAND5 - 2400 - OtherRelief;
        end;


        //message('%1-%2-%3-%4 -%5', PAYE);
        exit(PAYE);
    ...
    begin
        ...
        if ObjMembers.Get(MemberNo) then begin
            ...
            ObjMembers."Withdrawal Application Date" := Today;
            ObjMembers.Modify;
        end;
    ...
    begin
        ...
        if ObjLoans.FindSet then begin
            ...
            VarScheduleDateFilter := '..' + Format(CalcDate('-CM-1D', Today));
            ...
            if VarScheduleRepayDate <> 0D then begin
                VarDateFilter := '..' + Format(CalcDate('-CM-1D', Today));
    ...
    procedure FnCreateGnlJournalLineGuarantorRecovery(TemplateName: Text; BatchName: Text; DocumentNo: Code[30]; LineNo: Integer; TransactionType: Option " ","Registration Fee","Share Capital","Interest Paid","Loan Repayment","Deposit Contribution","Insurance Contribution","Benevolent Fund",Loan,"Unallocated Funds",Dividend,"FOSA Account"; AccountType: Enum "Gen. Journal Account Type"; AccountNo: Code[50];
                                                                                                                                                                                                                                                                                                                                                                       ...
                                                                                                                                                                                                                                                                                                                                                                       LoanNumber: Code[50];
                                                                                                                                                                                                                                                                                                                                                                       VarRecoveryType: Option Normal,"Guarantor Recoverd","Guarantor Paid";
                                                                                                                                                                                                                                                                                                                                                                       VarLoanRecovered: Code[20])
    var
        GenJournalLine: Record "Gen. Journal Line";
    begin
        GenJournalLine.Init;
                    ...
                    if ObjLoanGuarantors."Amont Guaranteed" > 0 then begin
                        ...
                        if ObjLoans.Get(ObjLoanGuarantors."Loan No") then begin
                            ...
                            if ObjLoans."Outstanding Balance" > 0 then begin
                                ...
                                ApprovedAmount := ObjLoans."Approved Amount";
                                if ObjLoanGuarantors."Total Loans Guaranteed" <> 0 then begin
                                    VarApportionedLiability := Round((VarLoanOutstandingBal / ApprovedAmount) * ObjLoanGuarantors."Amont Guaranteed", 0.5, '=');
    ...
    procedure FnCreateGnlJournalLineAtm(TemplateName: Text; BatchName: Text; DocumentNo: Code[30]; LineNo: Integer; TransactionType: Option " ","Registration Fee","Share Capital","Interest Paid","Loan Repayment","Deposit Contribution","Insurance Contribution","Benevolent Fund",Loan,"Unallocated Funds",Dividend,"FOSA Account"; AccountType: Enum "Gen. Journal Account Type"; AccountNo: Code[50];
                                                                                                                                                                                                                                                                                                                                                         ...
                                                                                                                                                                                                                                                                                                                                                         DimensionActivity: Code[40];
                                                                                                                                                                                                                                                                                                                                                         ExternalDocumentNo: Code[50];
                                                                                                                                                                                                                                                                                                                                                         TransactionDescription: Text[250];
                                                                                                                                                                                                                                                                                                                                                         LoanNumber: Code[50];
                                                                                                                                                                                                                                                                                                                                                         TraceID: Code[100])
    var
        GenJournalLine: Record "Gen. Journal Line";
    ...
    begin
        ...
        ObjLoans.SetRange(ObjLoans."Loan  No.", VarLoanNo);
        if ObjLoans.FindSet then begin
            ObjLoans.CalcFields(ObjLoans."Principle Paid to Date", ObjLoans."Interest Paid"
           );
    ...
    begin
        ...
        if ObjLoans.FindSet then begin
            ...
            VarPrinciplePaid := (ObjLoans."Principle Paid to Date") * -1;
            VarInterestPaid := (ObjLoans."Interest Paid");
            // VarInsurancePaid := ((ObjLoans."Loan Insurance Paid" * -1) + ObjLoans."Insurance Paid Historical");
        end;
    ...
    begin
        ...
        IF LoansRec.FIND('-') THEN BEGIN
            IF ((LoansRec."Issued Date" <> 0D) AND (LoansRec."Repayment Start Date" <> 0D)) THEN BEGIN
                ...
                LoanAmount := LoansRec."Approved Amount";
                InterestRate := LoansRec.Interest;
                //if InterestRate = 0 then InterestRate:=1 else InterestRate:=InterestRate;
                RepayPeriod := LoansRec.Installments;
                InitialInstal := LoansRec.Installments + LoansRec."Grace Period - Principle (M)";
                //LBalance:=LoansRec."Approved Amount";
                LBalance := LoansRec."Approved Amount";
                ...
                REPEAT
                    ...
                    IF LoansRec."Repayment Method" = LoansRec."Repayment Method"::"Reducing Balance" THEN BEGIN
                        ...
                        LoansRec.TESTFIELD(LoansRec.Installments);
                        LPrincipal := LoanAmount / RepayPeriod;
                        LInterest := Round((InterestRate / 1200), 1, '>') * LBalance;
                    END;
    ...
    procedure FnCreateFAGLJournalLineBalanced(TemplateName: Text; BatchName: Text; DocumentNo: Code[30]; LineNo: Integer; FAPostingType: enum "Gen. Journal Line FA Posting Type"; AccountType: enum "Gen. Journal Account Type";
                                                                                                                                             AccountNo: Code[50];
                                                                                                                                             TransactionDate: Date;
                                                                                                                                             TransactionDescription: Text;
    ...
    begin
        ...
        VarNewDate := Dmy2date(myMonth, myDate, myYear);
    end;
    ...
    procedure FnCreateGnlJournalLineSal(TemplateName: Text; BatchName: Text; DocumentNo: Code[30]; LineNo: Integer; TransactionType: Enum TransactionTypesEnum; AccountType: Enum "Gen. Journal Account Type";
                                                                                                                                         AccountNo: Code[50];
                                                                                                                                         TransactionDate: Date;
                                                                                                                                         TransactionAmount: Decimal;
                                                                                                                                         DimensionActivity: Code[40];
