Codeunit 80000 "AU Payroll Processing"
{
    ...
    var
        ...
        CoopParameters: Option "none",shares,loan,"loan Interest","Emergency loan","Emergency loan Interest","School Fees loan","School Fees loan Interest",Welfare,Pension,NSSF,Overtime,DevShare,NHIF,Holiday,"Benevolent Fund";
        PayrollType: Code[20];
    ...
    begin
        ...
        if SuspendPay = false then begin
            ...
            if curBasicPay > 0 then begin
                ...
                // prEmployeeTransactions.SetFilter(prEmployeeTransactions."Transaction Code", '%1', 'E0002');
                ...
                //     prEmployeeTransactions."Transaction Code" := 'E0002';
                //     prEmployeeTransactions."Transaction Name" := 'OverTime';
                //     prEmployeeTransactions."Original Amount" := ((curBasicPay * 12 * EmployeeP."Hours worked 2.0" * EmployeeP."Rate 2.0") / (2184)) + ((curBasicPay * 12 * EmployeeP."Hours worked 1.5" * EmployeeP."Rate 1.5") / (2184));//(EmployeehousingLeavy + curBasicPay);
                //     prEmployeeTransactions."Original Deduction Amount" := ((curBasicPay * 12 * EmployeeP."Hours worked 2.0" * EmployeeP."Rate 2.0") / (2184)) + ((curBasicPay * 12 * EmployeeP."Hours worked 1.5" * EmployeeP."Rate 1.5") / (2184));
                //     prEmployeeTransactions.Amount := ((curBasicPay * 12 * EmployeeP."Hours worked 2.0" * EmployeeP."Rate 2.0") / (2184)) + ((curBasicPay * 12 * EmployeeP."Hours worked 1.5" * EmployeeP."Rate 1.5") / (2184));
                //     prEmployeeTransactions."Amount(LCY)" := ((curBasicPay * 12 * EmployeeP."Hours worked 2.0" * EmployeeP."Rate 2.0") / (2184)) + ((curBasicPay * 12 * EmployeeP."Hours worked 1.5" * EmployeeP."Rate 1.5") / (2184));
                //     prEmployeeTransactions."Transaction Type" := prEmployeeTransactions."Transaction Type"::Income;
                //     if prEmployeeTransactions."Original Deduction Amount" > 0 then
                //         if not prEmployeeTransactions.Insert(true) then
                //             prEmployeeTransactions.Modify();

                // end;
                //end overtime
                prEmployeeTransactions.Reset;
                prEmployeeTransactions.SetRange("No.", strEmpCode);
                ...
                prEmployeeTransactions.SetFilter("Transaction Code", '%1', 'E0002');

                if prEmployeeTransactions.IsEmpty then begin

                    prEmployeeTransactions.Init();
                    prEmployeeTransactions."Entry No" := 0;
                end else begin

                    prEmployeeTransactions.FindLast();
                end;
                ...
                prEmployeeTransactions."Original Amount" :=
                    ((curBasicPay * 12 * EmployeeP."Hours worked 2.0" * EmployeeP."Rate 2.0") / 2184) +
                    ((curBasicPay * 12 * EmployeeP."Hours worked 1.5" * EmployeeP."Rate 1.5") / 2184);

                prEmployeeTransactions."Original Deduction Amount" := prEmployeeTransactions."Original Amount";
                prEmployeeTransactions.Amount := prEmployeeTransactions."Original Amount";
                prEmployeeTransactions."Amount(LCY)" := prEmployeeTransactions."Original Amount";


                if prEmployeeTransactions."Entry No" = 0 then
                ...
                prEmployeeTransactions.SetRange(prEmployeeTransactions."Period Year", intYear);
                //prEmployeeTransactions.SetRange(prEmployeeTransactions.Suspended, false);
                prEmployeeTransactions.SetFilter(prEmployeeTransactions."Transaction Code", '%1', 'HL');
                if prEmployeeTransactions.FindFirst() then begin
                    prEmployeeTransactions.DeleteAll(true);
                end;

                // Message('%1-%2-%3', curGrossPay, curTotAllowances, curSalaryArrears);

                EmployeehousingLeavy := 0;
                prTransactionCodes.RESET;
                prTransactionCodes.SETRANGE("Inclusive housing Leavy", TRUE);
                IF prTransactionCodes.FINDSET THEN BEGIN
                    REPEAT
                        ...
                        IF prEmployeeTransactions.FINDSET THEN BEGIN
                            REPEAT
                                EmployeehousingLeavy += prEmployeeTransactions.Amount;
                            UNTIL prEmployeeTransactions.NEXT() = 0;
                        END;
                ...
                if NOT prEmployeeTransactions.Find('-') then begin
                    ...
                    prEmployeeTransactions."Payroll Code" := PayrollCode;
                    prEmployeeTransactions."Payroll Period" := SelectedPeriod;
                    prEmployeeTransactions."Transaction Code" := 'HL';
                    prEmployeeTransactions."Transaction Name" := 'Housing Levy';
                    prEmployeeTransactions."Original Amount" := Zakayo / 100 * curGrossPay;
                    prEmployeeTransactions."Original Deduction Amount" := Zakayo / 100 * curGrossPay;
                    prEmployeeTransactions.Amount := Zakayo / 100 * curGrossPay;
                    prEmployeeTransactions."Amount(LCY)" := Zakayo / 100 * curGrossPay;
                ...
                if intNSSF_BasedOn = Intnssf_basedon::Gross then
                    curNssf_Base_Amount := curGrossPay;
                if intNSSF_BasedOn = Intnssf_basedon::Basic then
                    curNssf_Base_Amount := curBasicPay;

                If blnPaysNssf then
                    IF curGrossPay <= 9000 THEN BEGIN
                        curNSSF := 540;
                    END ELSE BEGIN
                        curNSSF := 540 + (curGrossPay - 9000) * 0.06;
                    END;

                IF curNSSF > 6480 THEN BEGIN
                    curNSSF := 6480;
                END;
                ...
                // Message('%1-%2-%3', curGrossPay, curBenefits, curValueOfQuarters);
               ...
               Coopparameters::none);
                ...
                if blnPaysNhif then begin
                    curNHIF := Round(curGrossPay * 0.0275, 1, '=');
                ...
                if curPensionStaff > curMaxPensionContrib then
                    curTaxablePay := curGrossTaxable - (curSalaryArrears + curDefinedContrib + curMaxPensionContrib + curOOI + curHOSP + curNonTaxable + CurrInsuranceRel + curNHIF + (Zakayo / 100 * curGrossPay))

                else
                    curTaxablePay := curGrossTaxable - (curSalaryArrears + curDefinedContrib + curPensionStaff + curOOI + curHOSP + curNonTaxable + CurrInsuranceRel + curNHIF + (Zakayo / 100 * curGrossPay));

                // Message('%1|%2|%3|%4|%5|%6|%7|%8', curTaxablePay, curGrossTaxable, curDefinedContrib, curMaxPensionContrib, curNonTaxable, CurrInsuranceRel, curNHIF, (Zakayo / 100 * curGrossPay));
                ...
                curTaxCharged := ROUND(fnGetEmployeePaye(curTaxablePay), 0.01, '=') - (curInsuranceReliefAmount + curReliefPersonal);
                //Message('%1|%2|%3|%4', ROUND(fnGetEmployeePaye(curTaxablePay)), curInsuranceReliefAmount, curReliefPersonal, CurrInsuranceRel);
                ...
                if (curReliefPersonal + curInsuranceReliefAmount) > curMaximumRelief then
                    curPAYE := (curTaxCharged - curMaximumRelief)// - NhifRelief
                else
                    curPAYE := (curTaxCharged);//- NhifRelief;
             ...
             and (prTransactionCodes."Deduct Premium" = false) then begin
                            ...
                            if (prTransactionCodes."Special Transaction" = prTransactionCodes."special transaction"::"Staff Loan") and
                               ...
                               (prTransactionCodes."Repayment Method" = prTransactionCodes."repayment method"::Amortized) then begin
                                curTransAmount := 0;
                                curLoanInt := 0;
                                curLoanInt := ROUND(prEmployeeTransactions."Interest Charged", 0.01, '=');
                ...
                curNetPay := curGrossPay - ROUND((curTotalDeductions + IsCashBenefit + Teltax2), 0.01, '=');


                curNetPay := ROUND(curNetPay, 0.01, '='); //- curExcessPension

                curNetPay := curNetPay - ROUND(curTotCompanyDed, 0.01, '='); //******Get Company Deduction*****
    ...
    begin
        ...
        //Message('speci%1', SpecialTranAmount);
    end;
    ...
    begin
        ...
        if prPAYE.FindFirst then begin
            ...
            //Message('ok %1', curTaxablePay);

            until prPAYE.Next = 0;
        end;
        PAYE := ROUND(PAYE, 0.01, '=');
    end;
    ...
    var
        ...
        ColumnLayout: Record "Column Layout";
        CalcAddCurr: Boolean;
        AccSchedMgt: Codeunit "Account schedule Line";
    //AccSchedMgt: Codeunit AccSchedManagement;
    begin
    ...
    procedure fnCalcLoanInterest(strEmpCode: Code[20]; strTransCode: Code[20]; InterestRate: Decimal; RecoveryMethod: Option Reducing,"Straight line",Amortized; LoanAmount: Decimal; Balance: Decimal; CurrPeriod: Date; IntAmount: Decimal) LnInterest: Decimal
    var
        curLoanInt: Decimal;
        intMonth: Integer;
        intYear: Integer;
