...
Table 172387 "Receipt Allocation"
{
    ...
    fields
    {
        ...
        {
            TableRelation = "Loans Register"."Loan  No." where("Client Code" = field("Account No"), "Outstanding Balance" = filter(<> 0));
        ...
        field(5; Amount; Decimal)
        {
            ...
            var
                ...
                Msg: Text;
                ConfirmAdjust: Label 'Amount entered (%1) is greater than the remaining %2 (%3).\Do you want to adjust it to %3?';
                AdjustedMsg: Label 'Amount has been adjusted to %1 (outstanding %2).';
            begin
                ...
                if ("Transaction Type" <> "Transaction Type"::Repayment) and
           ("Transaction Type" <> "Transaction Type"::"Interest Paid") then
                    exit;

                if "Loan No." = '' then
                    Error('Loan No. must be specified for Repayment or Interest Paid.');

                if not Loans.Get("Loan No.") then exit;

                Loans.CalcFields("Outstanding Balance", "Outstanding Interest");

                if "Transaction Type" = "Transaction Type"::Repayment then begin
                    RemainingPrincipal := Loans."Outstanding Balance";

                    if Amount > RemainingPrincipal then begin
                        if Confirm(ConfirmAdjust, false, Amount, 'loan balance', RemainingPrincipal, RemainingPrincipal) then begin
                            Amount := RemainingPrincipal;
                            Message(AdjustedMsg, Amount, 'loan balance');
                        end;
                ...
                if "Transaction Type" = "Transaction Type"::"Interest Paid" then begin
                    InterestDue := Loans."Outstanding Interest";

                    if Amount > InterestDue then begin
                        if Confirm(ConfirmAdjust, false, Amount, 'interest due', InterestDue, InterestDue) then begin
                            Amount := InterestDue;
                            Message(AdjustedMsg, Amount, 'interest');
                        end;
                    end;
                end;
        ...
        field(172150; "Global Dimension 1 Code"; Code[20])
        {
            TableRelation = "Dimension Value".Code where("Global Dimension No." = const(1));
        }
        field(172151; "Global Dimension 2 Code"; Code[20])
        {
            TableRelation = "Dimension Value".Code where("Global Dimension No." = const(2));
        }
        field(172152; "Cash Clearing Charge"; Decimal)
        {
        }
        field(172153; "Loan Outstanding Balance"; Decimal)
        {
        }
        field(172154; "Mpesa Account Type"; Option)
        {
        ...
        field(172155; "Mpesa Account No"; Code[20])
        {
        ...
        field(172156; "Cummulative Total Payment Loan"; Decimal)
        {
        }
        field(172157; "Loan Product Name"; Code[80])
        {
        }
        field(172158; "Total Allocation"; Decimal)
        {
            CalcFormula = sum("Receipt Allocation".Amount where("Document No" = field("Document No")));
            FieldClass = FlowField;
        }
        field(172160; "Account No"; Code[100])
        {
        ...
        field(172161; "Account Type"; enum "Gen. Journal Account Type")
        {
        ...
        field(172162; "Loan PayOff Amount"; Decimal)
        {
        }
        field(172163; Description; Text[100])
        {
        }
