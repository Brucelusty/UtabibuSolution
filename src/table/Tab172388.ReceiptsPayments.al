...
Table 172388 "Receipts & Payments"
{
    ...
    fields
    {
            ...
            begin
                ...
                if ("Account Type" = "account type"::Customer) or ("Account Type" = "account type"::Micro) then begin
                    if Mem.Get("Account No.") then
                        if (Mem.Status = Mem.Status::Exited) or (Mem.Status = Mem.Status::Deceased) then Error('Member status %1 does not allow transacting ', Mem.Status);
                    Name := Mem.Name;
                end;
        ...
        field(8; "Employer No."; Code[20])
        {
            ...
            TableRelation = if (Source = const(BOSA),
                                "Receipt Mode" = filter(Cheque | Cash | Mpesa | "Deposit Slip" | EFT)) "Bank Account"."No.";
            // else
            // if (Source = const(BOSA),
            //                     "Receipt Mode" = filter(Cash)) "Bank Account"."No." where("Bank Category" = filter(Cash))
            // else
            // if (Source = const(BOSA),
            //                     "Receipt Mode" = filter(Mpesa)) "Bank Account"."No." where("Bank Category" = filter('M-Pesa'));
        }
        ...
        {
            OptionCaption = 'Cash,Cheque,Mpesa,Deposit Slip,EFT';
            OptionMembers = Cash,Cheque,Mpesa,"Deposit Slip",EFT;
        }
        ...
        {
            OptionCaption = 'Deposit Contribution';
