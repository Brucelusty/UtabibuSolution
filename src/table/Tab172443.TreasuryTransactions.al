...
Table 172443 "Treasury Transactions"
{
    ...
    fields
    {
        ...
        {
            OptionCaption = 'Issue To Teller,Return To Treasury,Issue From Bank,Return To Bank,Inter Teller Transfers,End of Day Return to Treasury,Inter - Branch Treasury Transactions,HQ To Branch Treasury';
            OptionMembers = "Issue To Teller","Return To Treasury","Issue From Bank","Return To Bank","Inter Teller Transfers","End of Day Return to Treasury","Branch Treasury Transactions","Intra-Day to Treasury";
        ...
        {
            TableRelation = if ("Transaction Type" = filter("Issue To Teller" | "Return To Bank" | "Branch Treasury Transactions" | "Intra-Day to Treasury")) "Bank Account"."No." where("Account Type" = const(Treasury))
            else
            if ("Transaction Type" = filter("Return To Treasury" | "Return To Treasury" | "Inter Teller Transfers" | "End of Day Return to Treasury")) "Bank Account"."No." where("Account Type" = const(Cashier))
            else
        ...
        {
            TableRelation = if ("Transaction Type" = filter("Return To Treasury" | "Issue From Bank" | "Branch Treasury Transactions" | "End of Day Return to Treasury" | "Intra-Day to Treasury" )) "Bank Account"."No." where("Account Type" = const(Treasury))
            else
