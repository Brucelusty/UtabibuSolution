...
Table 172940 "Member Agent/Next Of Kin Chang"
{
    ...
    //     ObjMemberNextofKinChange.SetRange("Document No", DocumentNo);
    //     if ObjMemberNextofKinChange.FindSet then
    //         ObjMemberNextofKinChange.DeleteAll;
    ...
    //             ObjMemberNextofKinChange.Init;
    //             ObjMemberNextofKinChange."Document No" := "Document No";
    //             ObjMemberNextofKinChange."Member No" := "Member No";
    ...
    //             ObjMemberNextofKinChange.Relationship := ObjMemberNOKDetails.Relationship;
    //             ObjMemberNextofKinChange."Account No" := ObjMemberNOKDetails."Account No";
    ...
    //             ObjMemberNextofKinChange.Insert(true);
    ...
    var
        ObjMemberNextofKinChange: Record "Member NOK Change Request";
        TempCheck: Record "Member NOK Change Request";
        Exists: Boolean;
    begin
        ObjMemberNOKDetails.Reset();
        ObjMemberNOKDetails.SetRange("Account No", "Member No");

        if ObjMemberNOKDetails.FindSet() then begin
            repeat
                // Fully reset record variable
                ObjMemberNextofKinChange.RESET();

                // Check if record already exists
                Exists := false;
                TempCheck.RESET();
                TempCheck.SETRANGE("Document No", DocumentNo);
                TempCheck.SETRANGE("Account No", ObjMemberNOKDetails."Account No");
                if TempCheck.FINDFIRST() then
                    Exists := true;

                // Only insert if it does not exist
                if not Exists then begin
                    ObjMemberNextofKinChange.Init();
                    ObjMemberNextofKinChange."Document No" := DocumentNo;
                    ObjMemberNextofKinChange."Member No" := "Member No";
                    ObjMemberNextofKinChange."Account No" := ObjMemberNOKDetails."Account No";
                    ObjMemberNextofKinChange.Name := UpperCase(ObjMemberNOKDetails.Name);
                    ObjMemberNextofKinChange.Relationship := ObjMemberNOKDetails.Relationship;
                    ...
                    ObjMemberNextofKinChange."%Allocation" := ObjMemberNOKDetails."%Allocation";
                    ObjMemberNextofKinChange."Next Of Kin Type" := ObjMemberNOKDetails."Next Of Kin Type";
                    ObjMemberNextofKinChange."Date of Birth" := ObjMemberNOKDetails."Date of Birth";
                    ObjMemberNextofKinChange.INSERT();
                end;
        ...
        if ObjNOKAgentChange.FindSet then begin
            ...
            if ObjAccountAgentDetails.FindSet then begin
                repeat
                    ...
                    ObjMemberAccountAgent.Insert(true)
