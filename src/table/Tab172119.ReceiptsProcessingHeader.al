Table 172119 "Receipts Processing Header"
{
    // DrillDownPageID = "receipts processing history";
    // LookupPageID = "receipts processing history";
    ...
    fields
    {
        ...
        field(23; "Account No"; Code[30])
        {
            ...
            if ("Account Type" = const(Customer)) Customer where(ISNormalMember = const(false))
            else
        ...
        {
            CalcFormula = sum("Receipts Processing Lines".Amount where("Receipt Header No" = field(No)));
            Editable = false;
            FieldClass = FlowField;
        }
        field(27; "Total Count"; Integer)
        {
            CalcFormula = count("Receipts Processing Lines" where("Receipt Header No" = field(No)));
            FieldClass = FlowField;
        }
