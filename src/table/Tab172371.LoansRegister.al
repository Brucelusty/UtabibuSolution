...
Table 172371 "Loans Register"
{
    ...
    fields
    {
        ...
        field(3; "Loan Product Type"; Code[50])
        {
            ...
            begin
                ...
                if LoanType.Get("Loan Product Type") then begin
                    ...
                    // if "Loan Product Type" = 'M-SHOFCO' then error('This Loan product can only be applied through mobile');
                    if (LoanType."Loan Product Expiry Date" = 0D) or (LoanType."Loan Product Expiry Date" > Today) then begin
                        ...
                        if LoanApp.Find('-') then begin
                            repeat
                                ...
                                if LoanApp."Outstanding Balance" > 0 then begin
                                    if "Is Offset" = false then
                                        Error('Member has another Loan of same type. Loan Type name: %1 outstanding Balance %2 ', LoanApp."Loan Product Type", LoanApp."Outstanding Balance");
                                end;
                        ...
                        LoanApp.SetRange(LoanApp."Client Code", "Client Code");
                        LoanApp.SetRange(LoanApp.Posted, true);
                        LoanApp.setfilter("Loans Category", '%1|%2|%3', LoanApp."Loans Category"::Doubtful, LoanApp."Loans Category"::Substandard, LoanApp."Loans Category"::Loss);
                        if LoanApp.Find('-') then begin
                            repeat
                                LoanApp.CalcFields(LoanApp."Outstanding Balance");
                                if LoanApp."Outstanding Balance" > 0 then begin

                                    Message('Member has Loan with Category %1. Loan Type name: %2 outstanding Balance %3 ', LoanApp."Loans Category", LoanApp."Loan Product Type", LoanApp."Outstanding Balance");
                                end;
                            until LoanApp.Next = 0;
                        end;
                        ...
                        if LoanType.Get("Loan Product Type") then begin
                            ...
                            "Recommended Amount" := LoanType."Deposits Multiplier" * "Member Deposits";
        ...
        {
            TableRelation = if (Source = const(BOSA)) Customer."No." where("Account Category" = filter(Individual), ISNormalMember = filter(true))
            else
            ...
            begin
                ...
                if CustomerRecord.Get("BOSA No") then begin
                    // GenSetUp.Get();
                    // if CalcDate(GenSetUp."Loan Application Perion", CustomerRecord."Registration Date") > Today then
                    //     Error('Member has not met threshhold period');//ron
                    //if CustomerRecord.Status <> CustomerRecord.Status::Active then
                    // Error('Only Active member is allowed transacting. current member status %1' + Format(CustomerRecord.Status));
                end;
                ...
                if CustomerRecord.Get("BOSA No") then begin
                    ...
                    "Mobile Phone No" := CustomerRecord."Mobile Phone No";
                    "Account No" := CustomerRecord."FOSA Account No.";
                    "Group Code" := CustomerRecord."Group No";
        ...
        field(8; "Requested Amount"; Decimal)
        {
            ...
            begin
                // if LoanType.Get("Loan Product Type") then begin
                //     if "Requested Amount" < LoanType."Min. Loan Amount" then
                ...
                //Monthly Interest Formula PR(T+1)
                ...
                if "Repayment Method" = "repayment method"::Amortised then begin
                    ...
                    //  TotalMRepay:=ROUND((InterestRate/12/100) / (1 - POWER((1 + (InterestRate/12/100)),- RepayPeriod)) * "Requested Amount",1,'>');
        ...
        {
            OptionCaption = 'Application,Appraisal,Rejected,Disbursed,Closed,Approved,Open';
            OptionMembers = Application,Appraisal,Rejected,Disbursed,Closed,Approved,Open;
            ...
            begin
                ...
                begin
                    if "Loan Disbursement Date" < Today then
                        Error('Disbursement date can not be in the past.');
                end;
                ...
                IF DAY <= 10 THEN BEGIN
                    "Repayment Start Date" := CALCDATE('CM', "Loan Disbursement Date");
                END ELSE BEGIN
                    BaseCaldate := CALCDATE('CM+1D', "Loan Disbursement Date");
                    "Repayment Start Date" := CALCDATE('CM', BaseCaldate);
                END;
                if "Loan Product Type" = 'INSTANT' then begin
                    "Repayment Start Date" := "Loan Disbursement Date";
                end;

                if "Loan Product Type" = 'INSTANT' then begin
                    "Expected Date of Completion" := CalcDate('90D', "Repayment Start Date");
                end else begin
        ...
        {
            OptionCaption = ' ,FOSA Account,EFT,RTGS,Mpesa,Bank Transfer,Cheque';
            OptionMembers = " ","FOSA Account",EFT,RTGS,Mpesa,"Bank Transfer","Cheque";
        ...
        field(172300; "Outstanding Balance"; Decimal)
        {
            ...
            CalcFormula = sum("Detailed Cust. Ledg. Entry".Amount where("Loan No" = field("Loan  No."), "Transaction Type" = filter(Loan | Repayment), "Posting Date" = field("Date filter")));
            Editable = false;
            FieldClass = FlowField;
        ...
        field(53190; "Repayment Start Date"; Date)
        {
            ...
            begin
                ...
                if "Loan Product Type" = 'INSTANT' then begin
                    "Expected Date of Completion" := CalcDate('30D', "Repayment Start Date");
                end else begin
                    "Expected Date of Completion" := CalcDate(Format(Installments) + 'M', "Repayment Start Date");
                end;
            end;
        ...
        field(68057; Gender; Enum "Employee Gender")
        {
                ...
                if "Approval Status" = "Approval Status"::Approved then begin
                    ...
                    if Cust.Find('-') then begin
                        if Cust."Mobile Phone No" <> '' then
                            Message := '';
                        // Message := 'Dear' + Format(AU.SplitString(Cust.Name, '')) + ' , ' + 'Thank you for patronizing our loan products! ' + 'Your loan has been approved.' + 'Funds will be disbursed within 7 days.';
                        ...
                        // if Cust."Mobile Phone No" <> '' then
                        //     Message := '';
                        // Message := 'Dear' + Format(AU.SplitString(Cust.Name, '')) + ' , ' + 'Thank you for patronizing our loan products! ' + 'Your loan of Kes.' + Format(LoanAmount) + '-' + 'has been approved.' + 'Funds will be disbursed within 7 days. We wish to notify you that your expected monthly loan repayment is Kes.' + Format(Repayment) + ' for ' + Format(Installments);
                        // SmsManagement.SendSmsResponse(Cust."Mobile Phone No", Message);
                    end;
                end;
            end;
        ...
        field(69067; "Loans Insurance"; Decimal)
        {
            // CalcFormula = max("Cust. Ledger Entry"."Posting Date" where("Customer No." = field("Client Code"),
            //                                                               "Loan No" = field("Loan  No."),
            //                                                              "Transaction Type" = filter("Loans Insurance")));
            // FieldClass = FlowField;
        }
        ...
        field(172208; "Credit Officer"; Code[28])
        {
        ...
        field(172209; "Loan Centre"; Code[20])
        {
        }
        field(172210; "Loan Reschedule Instalments"; Integer)
        {
        ...
        field(172211; "Disburesment Type"; Option)
        {
            OptionCaption = ' ,Full/Single disbursement,Trunch/Multiple Disbursement';
        ...
        field(172212; "Amount to Disburse on Trunch 1"; Decimal)
        {
        ...
        field(172213; "No of Tranch Disbursment"; Integer)
        {
        }
        field(172214; "Loan Stages"; Code[10])
        {
        ...
        field(172215; "Loan Stage Description"; Text[30])
        {
        }
        field(172216; "Loan Current Payoff Amount"; Decimal)
        {
        }
        field(172217; "Outstanding Penalty"; Decimal)
        {
        }
        field(172218; "Tranch Amount Disbursed"; Decimal)
        {
        }
        field(172219; "Repayment Dates Rescheduled"; Boolean)
        {
        }
        field(172220; "Repayment Dates Rescheduled On"; Date)
        {
        }
        field(172221; "Repayment Dates Rescheduled By"; Code[20])
        {
        }
        field(172222; Closed; Boolean)
        {
        }
        field(172223; "Principle Paid to Date"; Decimal)
        {
            CalcFormula = sum("Detailed Cust. Ledg. Entry".Amount where("Customer No." = field("Client Code"),
                                                                  "Transaction Type" = filter(repayment),
                                                                  "Loan No" = field("Loan  No."),
        ...
        field(172224; "Days In Arrears"; Integer)
        {
        }
        field(172225; "New Repayment Start Date"; Date)
        {
        ...
        field(172226; "Loan Collateral Secured"; Boolean)
        {
            CalcFormula = exist("Loan Collateral Details" where("Loan No" = field("Loan  No.")));
            FieldClass = FlowField;
        }
        field(172227; "Loans Under CRB Notice"; Boolean)
        {
        }
        field(172228; "Loan Purpose Description"; Text[50])
        {
        }
        field(172229; "Credit Officer II"; Code[30])
        {
            TableRelation = "Credit/Field Officers"."User ID";
        }
        field(172230; "Date of Rejection"; Date)
        {
        }
        field(172231; "Intent to Reject"; Boolean)
        {
        }
        field(172232; "Except From Penalty"; Boolean)
        {
        }
        field(172233; "Loan Recovery Account FOSA"; Code[20])
        {
            TableRelation = Vendor."No." where("BOSA Account No" = field("Client Code"));
        }
        field(172234; "Loan Amount Due"; Decimal)
        {
        }
        field(172235; "Reschedule Effective Date"; Date)
        {
        ...
        field(172236; "Working Date"; Date)
        {
        }
        field(172237; "Excess LSA Recovery"; Boolean)
        {
        }
        field(172238; "Excess Ufalme Recovery"; Boolean)
        {
        }
        field(172239; "Current Principle Due"; Decimal)
        {
            Editable = false;
        }
        field(172240; "Current Interest Due"; Decimal)
        {
        }
        field(172241; "Current Insurance Due"; Decimal)
        {
        }
        field(172242; "Current Penalty Due"; Decimal)
        {
            CalcFormula = sum("Detailed Cust. Ledg. Entry".Amount where("Customer No." = field("Client Code"),
        ...
        field(172243; "Actual Loan Balance"; Decimal)
        {
            CalcFormula = sum("Detailed Cust. Ledg. Entry".Amount where("Customer No." = field("Client Code"),
        ...
        field(172244; "Closed On"; Date)
        {
        }
        field(172245; "Closed By"; Code[20])
        {
        }
        field(172246; "Principle Paid Historical"; Decimal)
        {
        ...
        field(172247; "Interest Paid Historical"; Decimal)
        {
            CalcFormula = sum("Member Loans Historical Ledger".Amount where("Account No." = field("Client Code"),
                                                                             "Loan No" = field("Loan  No."),
                                                                             "Transaction Type" = filter("Interest Paid"),
        ...
        field(172248; "Insurance Paid Historical"; Decimal)
        {
            CalcFormula = sum("Member Loans Historical Ledger".Amount where("Account No." = field("Client Code"),
                                                                             "Loan No" = field("Loan  No."),
        ...
        field(172249; "Penalty Paid Historical"; Decimal)
        {
            CalcFormula = sum("Member Loans Historical Ledger".Amount where("Account No." = field("Client Code"),
                                                                             "Loan No" = field("Loan  No."),
        ...
        field(172250; "Application type"; Option)
        {
            OptionCaption = 'System,Mobile';
            OptionMembers = System,Mobile;
        }
        field(172251; "Total Interest Paid"; Decimal)
        {
            Editable = false;
        }
        field(172252; "Total Insurance Paid"; Decimal)
        {
            Editable = false;
        }
        field(172253; "Total Penalty Paid"; Decimal)
        {
            Editable = false;
            CalcFormula = sum("Detailed Cust. Ledg. Entry".Amount where("Customer No." = field("Client Code"),
                                                                  "Transaction Type" = filter("Penalty Paid"),
        ...
        field(172254; "Actual Loan Balance Historical"; Decimal)
        {
        ...
        field(172255; "Corporate Loan"; Boolean)
        {
        }
        field(172256; "Loan Bal As At Debt Collection"; Decimal)
        {
        }
        field(172257; "Offset Eligibility Amount"; Decimal)
        {
        }
        field(172258; "Insider Status"; Option)
        {
            OptionCaption = ' ,Board Member,Staff Member';
            OptionMembers = " ","Board Member","Staff Member";
        }
        field(172259; "Member Deposit Account No"; Code[20])
        {
        }
        field(172260; "Freeze Interest Accrual"; Boolean)
        {
        }
        field(172261; "Freeze Until"; Date)
        {
        }
        field(172262; "Insurance Payoff"; Decimal)
        {
        }
        field(172263; "OneOff Loan Repayment"; Boolean)
        {
        }
        field(172264; "Repayment Due Date"; Date)
        {
        }
        field(172265; "Borrowing Within Deposits"; Boolean)
        {
        }
        field(172266; "Loan Installments to Skip"; Integer)
        {
        }
        field(172267; "Skip Installments Effected"; Boolean)
        {
        }
        field(172268; "Skip Installments Effected By"; Code[15])
        {
        }
        field(172269; "Skip Installments Effected On"; DateTime)
        {
        }
        field(172270; "Installment Date to Skip From"; Date)
        {
            TableRelation = "Loan Repayment Schedule"."Repayment Date" where("Loan No." = field("Loan  No."));
            //This property is currently not supported
            //TestTableRelation = false;
        ...
        field(172271; "Exempt From Payroll Deduction"; Boolean)
        {
        }
        field(172272; "Loan Missing in Portfolio Prov"; Boolean)
        {
        }
        field(172273; "Last Repayment Date"; Date)
        {
            CalcFormula = max("Cust. Ledger Entry"."Posting Date" where("Transaction Type" = filter(repayment),
                                                                          "Loan No" = field("Loan  No.")));
            FieldClass = FlowField;
        }
        field(172274; "Loan Auctioneer"; Code[10])
        {
        ...
        field(172275; "Loan Auctioneer Name"; Code[15])
        {
        }
        field(172276; "Date Auctioneer Assigned"; Date)
        {
        }
        field(172277; "Loan Under Auctioneer"; Boolean)
        {
        }
        field(172278; "Total Principle Paid"; Decimal)
        {
            CalcFormula = sum("Detailed Cust. Ledg. Entry".Amount where("Customer No." = field("Client Code"),
                                                                  "Loan No" = field("Loan  No."),
                                                                  "Transaction Type" = filter(repayment)));
        ...
        field(172279; "Member Credit Score"; Decimal)
        {
        }
        field(172280; "1 3rd of Basic"; Decimal)
        {
        }
        field(172281; "Bank Account No"; Code[15])
        {
            DataClassification = ToBeClassified;
        }
        field(172282; "Bank Branch Name"; Text[30])
        {
            DataClassification = ToBeClassified;
        }
        field(172283; "Insider-Board"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(172284; "Insider-Employee"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(172285; "Loan Principal Offset"; Decimal)
        {
            CalcFormula = sum("Loan Offset Details"."Principle Top Up" where("Loan No." = field("Loan  No."),
                                                                              "Client Code" = field("Client Code")));
            FieldClass = FlowField;
        }
        field(172286; "Interest Upfront"; Decimal)
        {
            DataClassification = ToBeClassified;
        }
        field(172287; "Main Sector"; Code[50])
        {
            // DataClassification = ToBeClassified;
            //TableRelation = "Main Sector".Code;
            CalcFormula = lookup("Sub Sector".No where(Code = field("Sub Sector")));
            Editable = false;
            FieldClass = FlowField;


        }
        field(172288; "Sub Sector"; Code[50])
        {
            //DataClassification = ToBeClassified;
            CalcFormula = lookup("Specific Sector".No where(Code = field("Sector Specific")));
            //Editable = false;
            FieldClass = FlowField;
            ...
            var
                ...
                specifics: Record "Specific Sector";
            begin

                SubSector.Reset();
                SubSector.SetRange(Code, specifics.No);
                if SubSector.Find() then begin
                    "Sub Sector" := SubSector.code;

                    Modify();
                    // Message('home %1', "Sub Sector");
                end;
        ...
        field(172289; "Sector Specific"; Code[20])
        {
            DataClassification = ToBeClassified;
            TableRelation = "Specific Sector".Code;
            trigger OnValidate()
            var
                SubSector: Record "Sub Sector";
                Mainsector: Record "Main Sector";
                specifics: Record "Specific Sector";
            begin
                CalcFields("Sub Sector", "Main Sector", "Sub-Sector Discription", "Specific-Sector Discription", "Main-Sector Discription");
                specifics.reset;
                SubSector.Reset();
                SubSector.SetRange(Code, specifics.No);
                if SubSector.Find() then begin
                    "Sub Sector" := SubSector.code;
                    Validate("Sub-Sector Discription");
                    Modify();
                    specifics.reset;
                    if specifics.Get("Sector Specific") then begin
                        "Sub-Sector Discription" := Specifics.Description;
                    end
        ...
        field(172290; "Sacco Decision"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(172291; "Due Loans"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        ...
        field(172017; "Specific-Sector Discription"; Text[2048])
        {


            CalcFormula = lookup("Specific Sector".Description where(Code = field("Sector Specific")));
            FieldClass = FlowField;
            Editable = false;
        }
        field(172018; "Sub-Sector Discription"; Text[2048])
        {


            CalcFormula = lookup("Sub Sector".Description where(Code = field("Sub Sector")));
            FieldClass = FlowField;
            Editable = false;
        }
        field(172019; "Main-Sector Discription"; Text[2048])
        {
            CalcFormula = lookup("Main Sector".Description where(Code = field("Main Sector")));
            FieldClass = FlowField;
            Editable = false;
        }
        field(172020; "Net Salary"; Decimal)
        {

        }
        field(172021; "Is Offset"; Boolean) { }
        field(172022; "Mobile Phone No"; Code[200]) { }
        field(172023; "View Schedule"; Boolean)
        {

        }
        field(172025; "ShareCapitalToDeduct"; Decimal)
        {

        }
        field(172024; "Deposit Contrib"; Decimal)
        {

        }
        field(172026; "Share Capital Contrib"; Decimal)
        {

        }
        field(172027; "Programmed Deposit Contrib"; Decimal)
        {

        }
        field(172028; "Housing Levy"; Decimal)
        {

        }
        field(172029; "Deposit Boosting"; Decimal)
        {
            trigger OnValidate()
            begin
                GenSetUp.Get();
                "Deposit Boosting Commision" := Round(GenSetUp."Deposit Boosting Rate" / 100 * "Deposit Boosting", 1, '>');
            end;
        }
        field(172030; "Deposit Boosting Commision"; Decimal)
        {
            Editable = false;
        }

        field(172031; "CRBQuerryCharge"; decimal)
        {

        }
        field(172032; Extended; Boolean)
        {

        }
        field(172033; "FOA Penalty Charged"; Boolean)
        {

        }
        field(172034; "FOA Penalty Date"; Date)
        {

        }

        field(172035; "Extended Reason"; Option)
        {
            OptionMembers = " ","Non-payment","Less Payment";
            OptionCaption = ' ,Non-payment Less Payment';

        }
        field(172036; "Extension Description"; Text[2048])
        {

        }
        field(172037; "CRB Metro"; Text[250])
        {

        }
        field(172038; "PPP1"; Text[250])
        {

        }
        field(172039; "Rating"; Option)
        {
            OptionMembers = " ","Good","Poor","Satisfactory","Default","Fair","Not Rated",;
            OptionCaption = ' ,Good, Poor, Satisfactory, Excellent, Default, Fair, Not Rated,';

        }
        field(172040; "Expected Repayments"; Decimal)
        {
            FieldClass = FlowField;
            CalcFormula = lookup("Loan Repayment Schedule"."Monthly Repayment" where("Loan No." = field("Loan  No.")));
        }
        field(172041; "Dividend Amount"; Decimal)
        {
            CalcFormula = - sum("Detailed Cust. Ledg. Entry".Amount where("Customer No." = field("Client Code"),
                                                                  "Transaction Type" = filter(Dividend),
                                                                  "Posting Date" = field("Date filter")));
        ...
        field(172042; "Default loan Attached"; Code[20])
        { }
        field(172043; "Extended Date"; Date)
        { }
        field(172044; "Reopening loan Remarks"; Text[30])
        { }
        field(172045; "Benevolent Fund Contrib"; Decimal)
        { }
        field(172046; "Mark As Performing"; Boolean)
        {
            trigger OnValidate()
            var
                myInt: Integer;
            begin
                IF Usersetup.GET(UserId) THEN BEGIN
                    IF Usersetup."Mark loan As Performing" = FALSE THEN
                        ERROR('You dont have permissions to Mark loan As performing, Please Contact your system Administrator!')
                END;
            End;

        }
        field(172047; AutoReversed; Boolean)
        {

        }
        field(172048; AutoReversedDate; DateTime)
        {

        }
        field(172049; "Defaulter Sms 1 Sent"; Boolean)
        { }
        field(172050; "Defaulter Sms 2 Sent"; Boolean)
        { }

        field(172051; "Disbused"; Boolean){
            FieldClass=FlowField;
            CalcFormula=exist("Mobile Banking Transactions" where (ReceiptNo=field("Doc No Used"), Response=filter(0)));
        }
    ...
    begin
        ...
        "Gross Pay" := Rec."Basic Pay H" + Rec."Medical AllowanceH" + Rec."House AllowanceH" + Rec."Other Income" + Rec."Transport Allowance";
        ...
        "Other Liabilities" + "Medical Insurance" + "Housing Levy" + "Pension Scheme";
