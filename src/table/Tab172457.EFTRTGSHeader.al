...
Table 172457 "EFT/RTGS Header"
{
    ...
    fields
    {
        ...
        field(17; RTGS; Boolean)
        {
            ...
            trigger OnValidate()
            begin
                if "Direct EFT" = true then Error('Uncheck the EFT inorder to use the RTGS mode');
                EFTDetails.Reset;
        ...
        field(25; Remarks; text[1024])
        {

        }
        field(26; "Direct EFT"; Boolean)
        {
            trigger OnValidate()
            begin
                if RTGS = true then Error('Uncheck the RTGS inorder to use EFT mode');
                EFTDetails.Reset;
                EFTDetails.SetRange(EFTDetails."Header No", No);
                if EFTDetails.Find('-') then begin
                    repeat
