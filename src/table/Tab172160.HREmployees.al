Table 172160 "HR Employees"
{
    ...
    fields
    {
        ...
        field(2010; "Leave Balance Days"; Decimal)
        {
            CalcFormula = sum("HR Leave Ledger Entries"."No. of days" where("Staff No." = field("No."),
                                                                             "Posting Date" = field("Date Filter"),
                                                                             "Leave Entry Type" = filter(Positive | Negative | Reimbursement),
                                                                              "Leave Type" = const('ANNUAL'),
                                                                             Closed = const(false)));
            FieldClass = FlowField;
        ...
        {
            CalcFormula = sum("HR Leave Ledger Entries"."No. of days" where("Leave Type" = const('DISCRETIONARY'),
                                                                             "Staff No." = field("No.")));
            FieldClass = FlowField;
        }
        ...
        field(53973; "Programme or Department Code"; Code[1000])
        {
            ...
            // Caption = 'Shortcut Dimension 3 Code';
            // Description = 'Stores the reference of the Third global dimension in the database';
            TableRelation = "Dimension Value".Code where("Dimension Code" = const('PROGRAMME'));
            trigger OnValidate()
            begin
                DimVal.RESET;
                //DimVal.SETRANGE(DimVal."Global Dimension No.",2);
        ...
        field(53974; "Employee Signature"; Blob)
        {
            DataClassification = ToBeClassified;
        }
    ...
    var
        ...
        objPayrollPeriod: Record "Payroll Calender.";
