...
Table 172113 "Payment Line"
{
    ...
    fields
    {
        ...
        field(3; Type; Code[20])
        {
            ...
            begin
                ...
                if PHead.FindFirst then begin
                    ...
                    "Pay Mode" := PHead."Pay Mode";
                    "Charge Mpesa" := PHead."Charge mpesa"
                end;
                ...
                if PLine.Get(No) then begin
                    ...
                    if PLine."Pay Mode" = PLine."pay mode"::Cheque then begin
                        ...
                        if PLine."Pay Mode" = PLine."pay mode"::EFT then begin
                            "Refund Charge" := Gen." EFT Charges PV"
                        end else
                            if PLine."Pay Mode" = PLine."pay mode"::RTGS then begin
                                "Refund Charge" := Gen."RTGS Charges PV"
                            end else
                                if ((PLine."Pay Mode" = PLine."pay mode"::Mpesa) and (PLine."Charge mpesa" = True)) then begin
                                    "Refund Charge" := Gen."M-Pesa Charges PV";
                                end;
                    // end;
                end;
                //
                if PLine.Get(No) then begin
                    if Type = 'BOARD' then begin
                        if PLine."Pay Mode" = PLine."pay mode"::Cheque then begin
                            "Refund Charge" := Gen."Loan Trasfer Fee-Cheque"
                        end;
        ...
        field(87; "Applies-to Doc. No."; Code[20])
        {
            ...
            begin
                ...
                with Rec do begin
                    ...
                    // OK:=
                    //  GenJournal.LookUpAppliesToDocVend(PayToVendorNo);
        ...
        field(172001; "Loan No."; Code[20])
        {
            TableRelation = if ("Transaction Type" = const(Loan)) "Loans Register"."Loan  No." where("Client Code" = field("Account No."));
        }
        field(172002; "Transaction Type"; Enum TransactionTypesEnum)
        {
            trigger OnValidate()
            begin
                if "Transaction Type" = "Transaction Type"::"Disbursment Account" then begin
                    Customer.Reset();
                    Customer.SetRange("No.", "Account No.");
                    if Customer.Find('-') then begin
                        Customer.CalcFields("Disbursment Account");
                        "Disbusment Amount" := Customer."Disbursment Account";
                    end;
                end;

            end;
        }
        field(172003; "Refund Charge"; Decimal)
        {
        }
        field(172004; "Mobile Phone No"; Code[2048])
        {
            // trigger OnValidate()
            // begin
            //     Customer.Reset();
            //     Customer.SetRange("Mobile Phone No", "Mobile Phone No");
            //     if Customer.Find('-') then begin
            //         Payee := Customer.Name;
            //     end;
            // end;
        ...
        field(172005; "Disbusment Amount"; Decimal)
        {
            Editable = false;
        }
        field(172006; "Charge Mpesa"; Boolean)
        {
    ...
    trigger OnDelete()
    begin
        //   Error('You cannot delete a record.');
