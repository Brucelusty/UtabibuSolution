...
Table 172363 "Member App Signatories"
{
    //nownPage51516363;
    //nownPage51516363;
    ...
    fields
    {
        ...
        field(7; "Must Sign"; Boolean)
        {
            ...
            var
                Signatries: Integer;
                MemberSignatory: Record "Member App Signatories";

            begin
                Signatries := 0;
                MemberSignatory.Reset();
                MemberSignatory.SetRange("Account No", "Account No");
                if MemberSignatory.FindSet() then begin
                    Signatries := MemberSignatory.Count;
                end;
                MembApp.Reset();
                MembApp.SetRange("No.", "Account No");
                if MembApp.FindFirst() then begin
                    if MembApp."Signing Instructions" = MembApp."Signing Instructions"::"Two to Sign" then begin
                        if Signatries > 2 then Error('Signatories Washatosha');
                    end else
                        if MembApp."Signing Instructions" = MembApp."Signing Instructions"::"Three to Sign" then begin
                            if Signatries > 3 then Error('Signatories Washatosha');
                        end else
                            if MembApp."Signing Instructions" = MembApp."Signing Instructions"::"Four to Sign" then begin
                                if Signatries > 4 then Error('Signatories Washatosha');
                            end
                            else
                                if MembApp."Signing Instructions" = MembApp."Signing Instructions"::"Sole Signatory" then begin
                                    if Signatries > 1 then Error('Signatories Washatosha');
                                end
