...
Table 172936 "Membership Cue"
{
    fields
    {
        ...
        {
            CalcFormula = count(Customer where(Status = const(Active), ISNormalMember = filter(true)));
            FieldClass = FlowField;

        }
        field(3; "Dormant Members"; Integer)
        {
            CalcFormula = count(Customer where(Status = const(Dormant), ISNormalMember = filter(true)));
            FieldClass = FlowField;
        }
        field(4; "Deceased Members"; Integer)
        {
            CalcFormula = count(Customer where(Status = const(Deceased), ISNormalMember = filter(true)));
            FieldClass = FlowField;
        }
        field(5; "Withdrawn Members"; Integer)
        {
            CalcFormula = count(Customer where(Status = const(Exited), ISNormalMember = filter(true)));
            FieldClass = FlowField;
        }
        ...
        {
            CalcFormula = count(Customer where(Status = const(Withdrawn), ISNormalMember = filter(true)));
            FieldClass = FlowField;
        }
        ...
        {
            CalcFormula = count("Loans Register" where("Loan Product Type" = const('DEV'),
                                                        Posted = const(true)));
            FieldClass = FlowField;
        }
        field(35; "Emergency Loan"; Integer)
        {
            CalcFormula = count("Loans Register" where("Loan Product Type" = const('EMERGENCY'),
                                                        Posted = const(true)));
            FieldClass = FlowField;
        }
        field(36; "Business Loan"; Integer)
        {
            CalcFormula = count("Loans Register" where("Loan Product Type" = const('COOP'),
                                                        Posted = const(true)));
            FieldClass = FlowField;
        }
        field(37; "Corporate Loan"; Integer)
        {
            CalcFormula = count("Loans Register" where("Loan Product Type" = const('DB'),
                                                        Posted = const(true)));
            FieldClass = FlowField;
        }
        field(38; "School Fees Loan"; Integer)
        {
            CalcFormula = count("Loans Register" where("Loan Product Type" = const('SCHOOL'),
                                                        Posted = const(true)));
            FieldClass = FlowField;
        }
        field(39; "Personal Loan"; Integer)
        {
            CalcFormula = count("Loans Register" where("Loan Product Type" = const('UTAPESA'),
                                                        Posted = const(true)));
            FieldClass = FlowField;
        }
        field(40; "Asset Finance"; Integer)
        {
            CalcFormula = count("Loans Register" where("Loan Product Type" = const('DEFAULTER'),
                                                        Posted = const(true),
        ...
        {
            CalcFormula = count("Loans Register" where("Loan Product Type" = const('DPLUS'),
                                                        Posted = const(true)));
            FieldClass = FlowField;
        }
        field(42; "Jijenge Loan"; Integer)
        {
            CalcFormula = count("Loans Register" where("Loan Product Type" = const('COOP41'),
                                                        Posted = const(true),
                                                        "Loan Status" = filter(Disbursed)));
            FieldClass = FlowField;
        }
        ...
        {
            CalcFormula = count("Loans Register" where("Loan Product Type" = const('INSTANT'),
                                                        Posted = const(true),
                                                        "Loan Status" = filter(Disbursed)));
            FieldClass = FlowField;
        }
        ...
        field(65; "House Hold Loan"; Integer)
        {
            CalcFormula = count("Loans Register" where("Loan Product Type" = const('REFINANCE'),
                                                        Posted = const(true),
                                                        "Loan Status" = filter(Disbursed)));
            FieldClass = FlowField;
        }

        field(66; "Haraka Loan"; Integer)
        {
            CalcFormula = count("Loans Register" where("Loan Product Type" = const('OVERDUE-INSTANT'),
                                                        Posted = const(true),
                                                        "Loan Status" = filter(Disbursed)));
            FieldClass = FlowField;
        }

        field(67; "Dhamana Loan"; Integer)
        {
            CalcFormula = count("Loans Register" where("Loan Product Type" = const('DIVIDEND'),
                                                        Posted = const(true),
                                                        "Loan Status" = filter(Disbursed)));
            FieldClass = FlowField;
        }

        field(68; "Defaulted Loans"; Integer)
        {
            CalcFormula = count("Loans Register" where("Loan Product Type" = const('JIBAMBE'),
                                                        Posted = const(true),
                                                        "Loan Status" = filter(Disbursed)));
            FieldClass = FlowField;
        }
        field(69; "Pamoja Loan"; Integer)
        {
            CalcFormula = count("Loans Register" where("Loan Product Type" = const('SHYLOCK'),
                                                        Posted = const(true),
                                                        "Loan Status" = filter(Disbursed)));
            FieldClass = FlowField;
        }
        field(70; "Qaribu Loan"; Integer)
        {
            CalcFormula = count("Loans Register" where("Loan Product Type" = const('SPE'),
                                                        Posted = const(true),
                                                        "Loan Status" = filter(Disbursed)));
            FieldClass = FlowField;
        }
        field(71; "Quick Fix Loan"; Integer)
        {
            CalcFormula = count("Loans Register" where("Loan Product Type" = const('SUPER'),
                                                        Posted = const(true),
                                                        "Loan Status" = filter(Disbursed)));
            FieldClass = FlowField;
        }
        field(72; "Super Premium Loan"; Integer)
        {
            CalcFormula = count("Loans Register" where("Loan Product Type" = const('SUPER'),
                                                        Posted = const(true),
                                                        "Loan Status" = filter(Disbursed)));
        ...
        field(73; "Vuka Loan"; Integer)
        {
            CalcFormula = count("Loans Register" where("Loan Product Type" = const('VK'),
                                                        Posted = const(true),
                                                        "Loan Status" = filter(Disbursed)));
            FieldClass = FlowField;
        }
        field(74; "X-Mass Loan"; Integer)
        {
            CalcFormula = count("Loans Register" where("Loan Product Type" = const('XM'),
                                                        Posted = const(true),
                                                        "Loan Status" = filter(Disbursed)));
            FieldClass = FlowField;
        }
        field(75; "Yasar Loan"; Integer)
        {
            CalcFormula = count("Loans Register" where("Loan Product Type" = const('YR'),
                                                        Posted = const(true),
                                                        "Loan Status" = filter(Disbursed)));
            FieldClass = FlowField;
        }
        field(76; "ALL Members"; Integer)
        {
            CalcFormula = count(Customer where(ISNormalMember = filter(true)));
            FieldClass = FlowField;
        }
        field(77; "Members Awaiting Exit"; Integer)
        {
            CalcFormula = count(Customer where(Status = filter('Awaiting Exit'), ISNormalMember = filter(true)));
            FieldClass = FlowField;

        }
        field(78;"Membership Application List";Integer)
        {
