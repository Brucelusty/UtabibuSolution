...
Report 172285 "Payroll Net Pay Transfer Ver1"
{
    ProcessingOnly = true;
    ...
    begin
        ObjPayrollPostingGroup.Get('UTABIBU');
        BATCH_TEMPLATE := 'GENERAL';
        BATCH_NAME := 'NETPAY';
        //DOCUMENT_NO := SFactory.FnRunGetNextTransactionDocumentNo;
        DocumentNExt := Format(Format(VarPayrollPeriod, 0,'<Month Text,3><Year4>'));
        ...
        //     GenJournalLine."account type"::Vendor, VarLoanSettlement, WorkDate, VarLoanRepayments * -1, 'BOSA', '',
        ...
        //     LineNo := LineNo + 10000;
        //     SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DOCUMENT_NO, LineNo, GenJournalLine."transaction type"::" ",
        //     GenJournalLine."account type"::Vendor, VarDepositsAccountNo, WorkDate, VarDepositsContribution * -1, 'BOSA', '',
        ...
        if ObjPayrollPeriodTransactions.FindSet then begin
            ...
            LineNo := LineNo + 10000;
            SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DocumentNExt, LineNo, GenJournalLine."transaction type"::" ",
            GenJournalLine."account type"::"G/L Account", ObjPayrollGeneralSetup."Staff Net Pay G/L Account", WorkDate, VarNETPay, 'BOSA', '',
        ...
        if ObjPayrollEmployee.FindSet then begin
            VarAmounttoDebitNetPay := VarNETPay;//VarLoanRepayments + VarDepositsContribution + VarNETPay;
            LineNo := LineNo + 10000;
            SFactory.FnCreateGnlJournalLine(BATCH_TEMPLATE, BATCH_NAME, DocumentNExt, LineNo, GenJournalLine."transaction type"::" ",
            GenJournalLine."account type"::"Bank Account", '', WorkDate, VarAmounttoDebitNetPay*-1, 'BOSA', '',
            'Net Pay For' + ' - ' + Format(VarPayrollPeriod, 0, '<Month Text,3> <Year4>'), '', GenJournalLine."application source"::CBS);
        end;
