report 172989 NWDTloansSasra
{

    RDLCLayout = 'Layouts/NWDTSasraReport.rdlc';
    DefaultLayout = RDLC;
    ApplicationArea = all;
    ...
    dataset
    {
        dataitem("Loans Register"; "Loans Register")
        {
            ...
            {}
            column(DateofBirth;Cust."Date Of Birth")
            {
            ...
            column(InterestRate; LoanProdType."Interest rate") { }
            ...
            begin
                ...
                // --- OVERDUE-INSTANT ---
                if "Loans Register"."Loan Product Type" = 'OVERDUE-INSTANT' then begin

                    if LoanProdType.Get("Loans Register"."Loan Product Type" = 'OVERDUE-INSTANT') then
                        LoanType := LoanProdType."Product Description";
                    // else
                    //     LoanType := 'OVERDUE-INSTANT';

                    // Expected Date of Completion: Issued Date + 90 days
                    if "Loans Register"."Issued Date" <> 0D then
                        "Loans Register"."Expected Date of Completion" :=
                            CalcDate('90D', "Loans Register"."Issued Date");

                    // Force Installments to 90
                    "Loans Register".Installments := 90;
                    "Loans Register"."Loan Product Type Name" := 'OVERDUE-INSTANT Loan'

                end

                // --- DEFAULTER ---
                else if "Loans Register"."Loan Product Type" = 'DEFAULTER' then begin
                    if LoanProdType.Get("Loans Register"."Loan Product Type") then
                        LoanType := LoanProdType."Product Description"
                    else
                        LoanType := 'DEFAULTER';

                    "Loans Register".Installments := 72;
                    if "Loans Register"."Issued Date" = 0D then
                        "Loans Register"."Issued Date" := "Loans Register"."Application Date";
                    if "Loans Register"."Issued Date" <> 0D then
                        "Loans Register"."Expected Date of Completion" :=
                            CalcDate('2160D', "Loans Register"."Issued Date");
                end
                ...
                // --- All other products ---
                    ...
                    if LoanProdType.Get("Loans Register"."Loan Product Type") then
                        ...
                        LoanType := "Loans Register"."Loan Product Type";
                end;
                // --- Approved Amount logic ---
                if ("Loans Register"."Approved Amount" = 0) then begin
                    PostedAmount := 0;
                    CustLedgEntry.Reset;
                    CustLedgEntry.SetRange("Document No.", 'OPENINGBAL-LOANS');
                    CustLedgEntry.SetRange("Loan No", "Loans Register"."Loan  No.");
                    CustLedgEntry.SetRange("Customer No.", "Loans Register"."Client Code");
                    if CustLedgEntry.FindSet() then begin
                        repeat
                            PostedAmount += CustLedgEntry.Amount;
                        until CustLedgEntry.Next() = 0;
                    end;

                    if PostedAmount <> 0 then
                        "Loans Register"."Approved Amount" := Abs(PostedAmount)
                    else if ("Loans Register"."Outstanding Balance" <> 0) then
                        "Loans Register"."Approved Amount" := "Loans Register"."Outstanding Balance";
                end;
            end;
        }
    }
