Report 80000 "Payroll JournalTransfer."
{
                ...
                if PeriodTrans.Find('-') then begin
                    repeat
                        ...
                        if PeriodTrans."Transaction Code" = 'NSSF' then begin
                            ...
                            GlobalDim1, '', CopyStr(PeriodTrans."Transaction Name" + '-' + PeriodTrans."Transaction Code", 1, 30), 0, PeriodTrans.Amount,
                        ...
                        if PeriodTrans."Transaction Code" = 'HL' then begin
                            GeneraljnlLine."Account Type" := GeneraljnlLine."account type"::"G/L Account";
                            //Credit Payables
                            CreateJnlEntry(0, '703041',
                            GlobalDim1, '', CopyStr(PeriodTrans."Transaction Name" + '-' + 'Payable' + '-' + PeriodTrans."Employee Code", 1, 150), 0, PeriodTrans.Amount,
                            PeriodTrans."Post As", '', SaccoTransactionType, GlobalDim3, "No.");

                            //Debit Staff Expense
                            CreateJnlEntry(0, '904026',
                            GlobalDim1, '', CopyStr(PeriodTrans."Transaction Name" + '-' + PeriodTrans."Employee Code", 1, 150), PeriodTrans.Amount, 0, 1, '',
                            SaccoTransactionType, GlobalDim3, "No.");
                        End;
                        ...
                        if PeriodTrans."Journal Account Type" <> PeriodTrans."Journal Account Type"::" " then begin
                            ...
                            if PeriodTrans."coop parameters" = PeriodTrans."coop parameters"::shares then begin
                                SaccoTransactionType := Saccotransactiontype::"Deposit Contribution";
                            end;


                            if PeriodTrans."coop parameters" = PeriodTrans."coop parameters"::Holiday then begin
                                SaccoTransactionType := Saccotransactiontype::"Programmed Deposits";

                            end;


                            if PeriodTrans."coop parameters" = PeriodTrans."coop parameters"::"Welfare" then begin
                                SaccoTransactionType := Saccotransactiontype::"Benevolent Fund";
                            end;
                            ...
                            if PeriodTrans."Transaction Code" = 'D0011' then begin
                                ...
                                if EmployerDed.Find('-') then begin
                                    ...
                                    GlobalDim1, '', CopyStr(PeriodTrans."Transaction Name" + '-' + 'Employee Payable' + '-' + PeriodTrans."Employee Code", 1, 150), 0,
                                    ...
                                    GlobalDim1, '', CopyStr(PeriodTrans."Transaction Name" + '-' + PeriodTrans."Employee Code", 1, 150), EmployerDed.Amount, 0, 1, '',
                                    SaccoTransactionType, GlobalDim3, "No.");
                                end;
    ...
    var
        ...
        SaccoTransactionType: enum TransactionTypesEnum;
        EmployerDed: Record "Payroll Employer Deductions.";
        GlobalDim3: Code[10];
        kk: Record "Payroll Calender.";
        UserSetup: Record "User Setup";
        DocumentNo: Label 'Salary';
        varPostingDate: Date;
        DocumentNExt: Text;


    procedure CreateJnlEntry(AccountType: Enum "Gen. Journal Account Type"; AccountNo: Code[20]; GlobalDime1: Code[20]; GlobalDime2: Code[20]; Description: Text[50]; DebitAmount: Decimal; CreditAmount: Decimal; PostAs: Option " ",Debit,Credit; LoanNo: Code[20]; TransType: Enum TransactionTypesEnum; BalAccountNo: Code[20]; MemberNo: Code[50])
    begin
        ...
        DocumentNExt := Format(PostingDate, 0, '<Month Text>');
        ...
        GeneraljnlLine."Document No." := DocumentNExt + DocumentNo;
        ...
        if PostAs = Postas::Debit then begin
            GeneraljnlLine."Debit Amount" := ROUND(DebitAmount);
            GeneraljnlLine.Validate("Debit Amount");
        end else begin
            GeneraljnlLine."Credit Amount" := ROUND(CreditAmount);
