report 80033 "Detailed Statement Via Email"
{
    ...
    dataset
    {
        dataitem(Customer; Customer)
        {
                ...
                dataitem(loan; "Detailed Cust. Ledg. Entry")
                {
                    ...
                    begin
                        if ((loan."Transaction Type" = loan."transaction type"::Loan) or (loan."Transaction Type" = loan."transaction type"::repayment)) or
                           ((loan."Transaction Type" = loan."transaction type"::"Interest Due") or (loan."Transaction Type" = loan."transaction type"::"Interest Paid")) or
                           ((loan."Transaction Type" = loan."transaction type"::"Penalty Charged") or (loan."Transaction Type" = loan."transaction type"::"Penalty Paid"))
                        then
            ...
            trigger OnPreDataItem();
            begin
                // Removed the conflicting error check for '005207'
                if CustomerNo = '' then
                    Error('Please specify a Customer No.');

                Customer.SetRange("No.", CustomerNo);
                if DateFilter = '' then
                    DateFilter := '01/01/23..' + Format(Today); // Hardcoded broad range for testing
                Customer.SetFilter("Date Filter", DateFilter);

                if Customer.IsEmpty() then
                    Message('No data found for Customer No. %1 with Date Filter %2. An empty statement may be generated.', CustomerNo, DateFilter);
            end;

            trigger OnAfterGetRecord();
            begin
                Message('Processing Customer: %1', "No."); // Debug message
    ...
    requestpage
    {
            ...
            area(content)
            {
                group(Options)
                {
                    ...
                    field("Customer No."; CustomerNo)
                    {
                        ApplicationArea = All;
                        Caption = 'Customer No.';
                        ToolTip = 'Specify the customer for the statement.';
                    }
                    field("Date Filter"; DateFilter)
                    {
                        ApplicationArea = All;
                        Caption = 'Date Filter';
                        ToolTip = 'Specify the date range for the statement.';
                    }
                    field(EmailAddress; EmailAddress)
                    {
                        ApplicationArea = All;
                        Caption = 'Email Address';
                        ToolTip = 'Specify the email address to send the statement to.';
                    }
                }
            }
        }
        ...
        trigger OnOpenPage()
        begin
            CustomerNo := '005207'; // Hardcoded for testing
            DateFilter := '01/01/23..' + Format(Today); // Hardcoded broad range
            EmailAddress := 'test@example.com'; // Hardcoded email for testing
        end;
    }
    ...
    local procedure SendStatementViaEmail()
    var
    ...
    begin
        ...
        else if Customer."E-Mail" <> '' then
            Recipient := Customer."E-Mail"
        else
            Error('No email address provided, and no email found in the customer record for %1.', Customer."No.");

        if Customer.IsEmpty() then
            Error('No data to generate report for Customer No. %1 with Date Filter %2', CustomerNo, DateFilter);

        FileName := StrSubstNo('MemberStatement_%1_%2.pdf', Customer."No.", Format(Today, 0, 'yyyyMMdd'));
        TempBlob.CreateOutStream(OutStream);
        Report.SaveAs(Report::"Detailed Statement Via Email", '', ReportFormat::Pdf, OutStream);

        TempBlob.CreateInStream(InStream);
        Subject := StrSubstNo('Your Account Statement as of %1', Format(Today));
        Body := StrSubstNo('Dear %1,\n\nPlease find attached your account statement for the period %2.\n\nBest regards,\n%3',
            Customer.Name, DateFilter, Company.Name);

        EmailMessage.Create(Recipient, Subject, Body, true);
        EmailMessage.AddAttachment(FileName, 'application/pdf', InStream);
        Email.Send(EmailMessage);

        Message('Statement sent successfully to %1.', Recipient);
    end;
    ...
    procedure SetEmailAddress(NewEmailAddress: Text[250])
    begin
