//<---------------------------------------------------------------------->															
Report 80004 "Member Dividends Report"
{
    RDLCLayout = 'Layouts/MemberDividendsReport.rdlc';
    DefaultLayout = RDLC;
    ...
    dataset
    {
        ...
        {
            CalcFields = "Current Shares", "Shares Retained", "Dividend Loan Balance", "dividend Amount";
            PrintOnlyIfDetail = false;
            RequestFilterFields = "No.", Status, "Registration Date", "Shares Retained", "dividend Amount", "Current Shares", "Global Dimension 2 Code", "Account Category";
            DataItemTableView = where(ISNormalMember = filter(true), "dividend Amount" = filter(<> 0), "Account Category" = filter(Individual));
            ...
            column(ShareCapitalAreas; ShareCapitalAreas)
            {

            }
            column(NetDivPayable; NetDivPayable)
            {
            ...
            column(Dividend_Loan_Balance; "Dividend Loan Balance")
            {
            }
            ...
            begin
                EntryNo := EntryNo + 1;
                Datefilter := '01012000' + '..' + Format(Today);
                //MESSAGE(Datefilter);															
                cust.Reset;
                ...
                if cust.FindSet then begin
                    ...
                    ShareCapitalAreas := 15000 - cust."Shares Retained";
                    if ShareCapitalAreas < 0 then begin
                        ShareCapitalAreas := 0;
                    end else begin
                        if ShareCapitalAreas > "Dividend Amount" then
                            ShareCapitalAreas := "Dividend Amount";
                ...
                if cust.FindSet then begin
                    ...
                    NetDivPayable := "Dividend Amount" - ("Dividend Loan Balance" + "ShareCapitalAreas");

                    if NetDivPayable < 0 then NetDivPayable := 0;
                end;
