...
Report 50084 "Loan Appraisal Draft"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Loan Appraisal DraftT.rdlc';
    ...
    dataset
    {
        dataitem("Loans Register"; "Loans Register")
        {
            ...
            column(Deposit_Boosting; "Deposit Boosting") { }
            column(Deposit_Boosting_Commision; "Deposit Boosting Commision") { }
            column(Security; VarTotalLoansnotSecuredbyCollateral) { }
            column(MpesaChargeAmount; MpesaChargeAmount) { }
            column(EFTAmount; EFTAmount) { }
            column(RTGSAmount; RTGSAmount) { }
            column(Deposit_Contrib; "Deposit Contrib") { }
            column(Share_Capital_Contrib; "Share Capital Contrib") { }
            column(Programmed_Deposit_Contrib; "Programmed Deposit Contrib") { }
            column(Benevolent_Fund_Contrib; "Benevolent Fund Contrib") { }
            column(COMPANYNAME; COMPANYNAME)
            {
            }
            column(Risk_MGT; "Risk MGT") { }
            column(Pension_Scheme; "Pension Scheme") { }
            ...
            column(CRB_Metro; "Loans Register"."CRB Metro")
            {

            }
            column(PPP1; "Loans Register".PPP1)
            {

            }
            column(Rating; "Loans Register".Rating)
            {
            ...
            dataitem(Customer; Customer)
            {
                ...
                column(Benevolent_Fund; "Benevolent Fund") { }
            }
            dataitem(LoansRegister; "Loans Register")
            {
                ...
                { }
                column(Repayment_Start_Date1; "Repayment Start Date")
                {
            ...
            dataitem("Loans Guarantee Details"; "Loans Guarantee Details")
            {
                ...
                // {
                // }
                column(Free_Shares; "Free Shares")
                {
            ...
            dataitem("Loan Collateral Details"; "Loan Collateral Details")
            {
                ...
                column(Security_Details; "Security Details") { }
                column(Asset_Value; "Asset Value") { }
                column(Code; Code) { }
                ...
                column(Collateral_Registe_Doc; "Collateral Registe Doc") { }
                ...
                column(Registration_Reference_No; "Registration/Reference No") { }
                column(Registered_Owner; "Registered Owner") { }
            ...
            begin
                ...
                Recomm := ROUND(FnReccommendAmount("Loans Register"."Requested Amount", DepX, TotalGuaranteed, VarTotalLoansnotSecuredbyCollateral, ROUND(Psalary, 0.05, '<')), 100, '<');
                ...
                if "Approved Amount" <> 0 then begin
                    ...
                    if "Loans Register"."loan product Type" <> 'QF' then begin
                ...
                if ("Loans Register"."loan product Type" = 'DIVIDEND') then begin
                    "Approved Amount" := "Loans Register"."Requested Amount";
                    // "Loans Register".Validate("Approved Amount", ROUND("Approved Amount", 100, '<'));
                end;
    ...
    var
        ...
        repayAmount: Decimal;
        Multiplier: Decimal;
        MiniumSharecapital: Decimal;
        ShareCapitalToDeduct: Decimal;
        bLaBalance: Decimal;
    ...
    procedure ShareCapital()
    begin
        ShareCap := 0;
        ShareCapitalToDeduct := 0;
        if Cust.Get("Loans Register"."Client Code") then begin
            ...
            if ShareCap < GenSetUp."Retained Shares" then begin
                ShareCapitalToDeduct := GenSetUp."Retained Shares" - ShareCap;
                "Loans Register".ShareCapitalToDeduct := ShareCapitalToDeduct;
    ...
    begin
        ...
        if LoanType.Get("Loans Register"."Loan Product Type") then begin
            ...
            if ObjCust.Get("Loans Register"."Client Code") then begin
                ...
                if (ObjCust."Current Shares" <> 0) then begin
                    ...
                    DEpMultiplier := depositMultiplier * (ObjCust."Current Shares" + "Loans Register"."Deposit Boosting") + "Loans Register"."Deposit Boosting" * GenSetUp."Deposits Multiplier";//"Loans Register"."Boosted Amount"+															
            ...
            LoanTopUp.SetRange(LoanTopUp."Loan No.", "Loans Register"."Loan  No.");
            LoanTopUp.SetRange(LoanTopUp."Client Code", "Loans Register"."Client Code");
            LoanTopUp.SetAutoCalcFields("Total Commission");
            if LoanTopUp.Find('-') then begin
                TopUpComm := LoanTopUp."Total Commission";

                //TotalBridgeAmount := ROUND(TotalBridgeAmount + LoanTopUp."Total Commission", 0.01, '>');
                //Message('%1', TopUpComm);
            ...
            ObjLoans.SetRange(ObjLoans."Client Code", "Loans Register"."Client Code");
            ObjLoans.SetFilter(ObjLoans."Outstanding Balance", '>%1', 0);
            ObjLoans.setfilter("Loan Product Type", '<>%1', 'INSTANT');
            if ObjLoans.FindSet then begin
                repeat
        ...
        ObjLoans.setfilter("Loan Product Type", '<>%1', 'INSTANT');
        if ObjLoans.FindSet then begin
            LOANBALANCE := LOANBALANCE - ObjLoans."Outstanding Balance";
        end;
        LoanG.Reset;
        LoanG.SetRange(LoanG."Loan No", "Loans Register"."Loan  No.");
        //LoanG.setrange("Self Guarantee", false);
        if LoanG.Find('-') then begin
            GurCount := LoanG.count;
            if GurCount = 1 then begin
                if LoanG."Self Guarantee" then begin
                    GurCount := 1
                end else
                    GurCount := 0;
            end;
        ...
        if GurCount = 1 then begin
            DepositSelf := "Loans Register"."Member Deposits" * 0.8;

        end;

        TotalLoanBal := LBalance;
        DepositValue := (DEpMultiplier) - LOANBALANCE;
        if DepositSelf > DepositValue then begin
            DepX := DepositSelf;
        end else begin
            DepX := DepositValue;
        end;
    ...
    begin
        ...
        Psalary := "Loans Register"."Net Utilizable Amount" * "Loans Register".Installments;

        if "Loans Register"."Loan Product Type" = 'INSTANT' then begin
            Psalary := "Loans Register"."Net Salary" * "Loans Register".Installments;
        end;
        //Message('rrr%1-%2-%3', Psalary, "Loans Register"."Net Utilizable Amount", "Loans Register".Installments);
    end;
    ...
    begin
        ...
        if ObjSecurities.Find('-') = true then begin

            VarTotalLoansnotSecuredbyCollateral := VarTotalLoansnotSecuredbyCollateral + ObjSecurities."Guarantee Value";
        end;
    ...
    begin
        ...
        if LoanType.Get("Loans Register"."Loan Product Type") then begin
            ...
            GenSetUp.Get();
            if "Loans Register"."Mode of Disbursement" = "Loans Register"."Mode of Disbursement"::EFT then begin
                EFTAmount := GenSetUp."Loan Trasfer Fee-EFT";

            end;
            if "Loans Register"."Mode of Disbursement" = "Loans Register"."Mode of Disbursement"::RTGS then begin
                RTGSAmount := GenSetUp."Loan Trasfer Fee-RTGS";

            end;
            if "Loans Register"."Mode of Disbursement" = "Loans Register"."Mode of Disbursement"::Mpesa then begin

                if "Loans Register"."Approved Amount" <= 10000 then begin
            ...
            LAppraisalFee := 0;
            LoanTransferFee := 0;
            LoanInsurance := Round(SFactory.FnGetChargeFee("Loans Register"."Loan Product Type", "Loans Register"."Approved Amount", 'CRB'), 1, '>');
            ProcessingFee := Round(SFactory.FnGetChargeFee("Loans Register"."Loan Product Type", "Loans Register"."Approved Amount", 'PROCESSING'), 1, '>');
            LoanFormFee := Round(SFactory.FnGetChargeFee("Loans Register"."Loan Product Type", "Loans Register"."Approved Amount", 'SMS') * ("Loans Register".Installments + 1), 1, '>');
            LAppraisalFee := Round(SFactory.FnGetChargeFee("Loans Register"."Loan Product Type", "Loans Register"."Approved Amount", 'UPFRONT'), 1, '>');
            ...
            Upfronts := LoanInsurance + CICInsurance + EmergencyUpfront + ProcessingFee + LoanFormFee + LAppraisalFee + BridgeLevy + ShareBoostComm + ExciseDutyShareBoostComm + "Loans Register"."Boosted Amount" +
            "Loans Register"."Boosted Amount Interest" + TotalBridgeAmount + MpesaChargeAmount + EFTAmount + RTGSAmount + "Loans Register"."Share Capital Contrib" + "Loans Register"."Deposit Contrib" + "Loans Register"."Programmed Deposit Contrib" + "Loans Register".CRBQuerryCharge
            + "Loans Register"."Deposit Boosting Commision" + "Loans Register"."Deposit Boosting" + "Loans Register"."Benevolent Fund Contrib";// + "Loans Register"."Boosting Commision";//+ TopUpComm;

            //END------------------------------------------------------------------------------------------------------------//															
            //"Loans Register".CalcFields("Loans Register"."Top Up Amount");															
            "Loans Register"."Loan SMS Fee" := SMSFEE;
            "Loans Register".Modify;
            Netdisbursed := "Loans Register"."Approved Amount" - Upfronts;// + TopUpComm;
        end;
        ...
        "Loans Register"."Boosting Commision" := TopUpComm;
        "Loans Register"."Total Offset Amount" := TotalBridgeAmount;
        "Loans Register".Modify;

        if "Loans Register"."Loan Product Type" = 'QF' then
            Netdisbursed := 0
        else
            Netdisbursed := Netdisbursed;
        // MESSAGE('%1|%2|%3', TotalBridgeAmount, Upfronts, TopUpComm);
    end;

    local procedure FnReccommendAmount(RequestedAmount: Decimal; QShares: Decimal; QGuarantors: Decimal; Securities: Decimal; QSalary: Decimal) RecommendedAmount: Decimal
    var
        GurCount: integer;
    begin
        ...
        if LoanType.Get("Loans Register"."Loan Product Type") then begin
            ...
            if (LoanType."Appraise Guarantors") or (LoanType."Appraise Collateral") then begin
                if RecommendedAmount > (QGuarantors + Securities) then
                    RecommendedAmount := (QGuarantors + Securities);
                //Message('sunday 4', RecommendedAmount);
            end;

            if (LoanType."Appraise Collateral") or (LoanType."Appraise Guarantors") then begin
                if RecommendedAmount > (Securities + QGuarantors) then
                    RecommendedAmount := QGuarantors + Securities;
            ...
            if (LoanType."Appraise Collateral") then begin
                if RecommendedAmount > (Securities + QGuarantors) then
                    RecommendedAmount := QGuarantors + Securities;
                // Message('sunday 4 -%1 -%2', RecommendedAmount, Securities);
            end;
            ...
            if LoanType."Appraise Salary" then begin
                ...
                //Message('sunday 5 %1- %2', RecommendedAmount, QSalary);
            end;
            ...
            if ((LoanType."Appraise Guarantors") and (LoanType."Appraise Salary") and (LoanType."Appraise Deposits")) then begin
                if RecommendedAmount > QSalary then begin
                    RecommendedAmount := QSalary;
                end else
                    ...
                    if RecommendedAmount > QShares then begin
                        ...
                        if RecommendedAmount > QGuarantors + securities then begin
                            RecommendedAmount := QGuarantors + securities;
                        end;

                Message('Quarantors %1-salary %2- deposits%3', QGuarantors, QSalary, QShares);
            end;
        end;
        if RecommendedAmount > RequestedAmount then begin
            RecommendedAmount := RequestedAmount;

        end;

        if RecommendedAmount < 0 then begin
            RecommendedAmount := 0;
        end;
        // Message('Quarantors %1-salary %2- deposits%3 - %4', QGuarantors, QSalary, QShares, RecommendedAmount);
    ...
    begin
        ...
        if "Loans Register"."Repayment Method" = "Loans Register"."repayment method"::"Straight Line" then begin
            ...
            LInterest := ROUND(((InterestRate / 1200) * LoanAmount) / "Loans Register".Installments, 1, '>');
        ...
        if "Loans Register"."Repayment Method" = "Loans Register"."repayment method"::Amortised then begin
            ...
            //Message('%1-%2', "Loans Register"."Loan Principle Repayment", "Loans Register"."Loan Interest Repayment");
